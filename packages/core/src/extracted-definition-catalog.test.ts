...
function fixture() {
  return {
    ...
    terms: [{ id: 'fixture.alpha', title: 'Альфа', kind: 'criterion_set', aliases: [], blockIds: [1, 2], itemBlocks: [2], coverage: 'criterion-list', note: 'Историческая редакция источника; требует проверки.' }],
  };
}

describe('source-excerpt catalog boundary', () => {
  it('preserves source text, nested/list numbering and qualifications', () => {
    const raw = fixture();
    const term = parseDefinitionCatalog(raw).terms[0];
  ...
  it('finds an owner definition without an invented external source URL', () => {
    const hit = createDefinitionLookup(fixture()).search('первое условие')[0];
  ...
  it('keeps owner source details and review state', () => {
    ...
    expect(catalog.sources[0]?.fileName).toBe('fixture.pdf');
  ...
  it('rejects publication promotion', () => {
    ...
    raw.sources[0]!.releaseEligible = true;
    expect(() => parseDefinitionCatalog(raw)).toThrow();
  });
  it('rejects dangling source and block references', () => {
    const raw = fixture();
    raw.blocks[0]!.source = 999;
    expect(() => parseDefinitionCatalog(raw)).toThrow();
    const another = fixture();
    another.terms[0]!.blockIds.push(999);
    expect(() => parseDefinitionCatalog(another)).toThrow();
  });
  it('rejects incomplete list/detail block membership', () => {
    const raw = fixture();
    raw.terms[0]!.itemBlocks.push(999);
    expect(() => parseDefinitionCatalog(raw)).toThrow();
  });
  it('rejects duplicate term and block identities', () => {
    const raw = fixture();
    raw.terms.push(raw.terms[0]!);
    expect(() => parseDefinitionCatalog(raw)).toThrow();
    const another = fixture();
    another.blocks.push(another.blocks[0]!);
    expect(() => parseDefinitionCatalog(another)).toThrow();
  });
  it('does not turn a mere scale mention into a calculator', () => {
    const raw = fixture();
    raw.terms[0]!.kind = 'scale';
    raw.terms[0]!.coverage = 'mention-only';
    const term = parseDefinitionCatalog(raw).terms[0];
  ...
  it('rejects owner URLs and local path traversal', () => {
    const raw = fixture();
    raw.sources[0]!.baseUrl = 'https://evil.example/';
    expect(() => parseDefinitionCatalog(raw)).toThrow();
    const another = fixture();
    another.sources[0]!.fileName = '../private.pdf';
    expect(() => parseDefinitionCatalog(another)).toThrow();
  });
  it('rejects missing source checksums and oversized input', () => {
    const raw = fixture();
    raw.sources[0]!.sourceSha256 = 'invalid';
    expect(() => parseDefinitionCatalog(raw)).toThrow();
