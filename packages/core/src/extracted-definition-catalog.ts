...
function object(value: unknown): Readonly<Record<string, unknown>> {
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new Error('Invalid excerpt object.');
...
function text(value: unknown, maximum = 2048, empty = false): string {
  if (typeof value !== 'string' || value.length > maximum || (!empty && !value.trim()))
    throw new Error('Invalid excerpt text.');
  return value;
}
function array(value: unknown, maximum: number): readonly unknown[] {
  if (!Array.isArray(value) || value.length > maximum)
    throw new Error('Invalid excerpt collection.');
  return value;
}
function integer(value: unknown): number {
  if (typeof value !== 'number' || !Number.isSafeInteger(value) || value <= 0)
    throw new Error('Invalid excerpt reference.');
  return value;
}
function safePath(source: DefinitionSource, path: string): void {
  ...
  if (
    ...
    throw new Error('Unsafe excerpt source path.');
}
...
export function parseExtractedDefinitionCatalog(input: unknown): DefinitionCatalog {
  ...
  if (!serialized || new TextEncoder().encode(serialized).length > MAX_BYTES)
    throw new Error('Excerpt module exceeds its size budget.');
  const root = object(input);
  if (
    root['version'] !== 3 ||
    root['textKind'] !== 'source-excerpt' ||
    root['reviewStatus'] !== 'requires-review' ||
    root['publicationState'] !== 'local-dev'
  )
    throw new Error('Unsupported excerpt module or trust state.');
  text(root['id'], 256);
  ...
  for (const entry of array(root['sources'], 1000)) {
    ...
    if (sourceIds.has(id) || row['releaseEligible'] !== false)
      throw new Error('Duplicate or release-eligible excerpt source.');
    ...
    if (
      ...
      throw new Error('Invalid excerpt authority.');
    ...
    const source: DefinitionSource = {
      ...
      accessed,
      sourceType,
      rightsStatus: text(row['rightsStatus'], 256),
    };
    ...
    if (sourceType === 'owner-pdf') {
      ...
      if (baseUrl || /[/\\]/u.test(fileName) || !source.sourceSha256)
        throw new Error('Invalid owner PDF provenance.');
  ...
  for (const entry of array(root['blocks'], 100000)) {
    ...
    const path =
      row['path'] === undefined ? (sourcePaths.get(source) ?? '') : text(row['path'], 2048, true);
    ...
    blocks.set(id, {
      source,
      text: text(row['text'], 262144),
      path,
      locator: text(row['locator'], 4096),
    });
  }
  const origins = array(root['etymologyStatements'] ?? [], 10000).map((entry) => {
    const row = object(entry);
    if (!blocks.has(integer(row['block'])) || row['reviewStatus'] !== 'requires-review')
      throw new Error('Invalid source etymology reference.');
    const block = blocks.get(integer(row['block']));
    ...
    if (
      ...
      [...block.text].slice(start, end).join('') !== statement
    )
      throw new Error('Etymology does not match the cited source span.');
    return statement;
  });
  ...
  const terms = array(root['terms'], 50000).map((entry): DraftDefinition => {
    ...
    if (
      ...
      throw new Error('Invalid excerpt term identity/kind/coverage.');
    ids.add(id);
    ...
    if (definition.length > 262144)
      throw new Error('Definition exceeds its display budget; split explicitly.');
    ...
    const etymology = array(row['etymologyRefs'] ?? [], 32).map((value) => {
      if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0 || !origins[value])
        throw new Error('Dangling etymology reference.');
      const statement = origins[value];
      if (statement === undefined) throw new Error('Missing etymology statement.');
      return statement;
    });
    const caveats = array(row['sourceCaveats'] ?? [], 30).map((entry) => {
    ...
    return {
      id,
      title: text(row['title'], 2048),
      kind: kind as DraftDefinition['kind'],
      aliases: array(row['aliases'] ?? [], 128).map((value) => text(value, 2048)),
      definition,
      items: [],
      note,
      coverage,
      etymology,
      references: selected.map((block) => ({
        source: block.source,
        path: block.path,
