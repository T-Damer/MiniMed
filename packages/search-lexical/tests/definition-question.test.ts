...
describe('definition question framing', () => {
  ...
  it.each(['', 'анизокория', 'у пациента нет икоты', 'Врач спросил что такое икота', 'Что такое?', 'Что такое', 'Что такое \0икота', 'x'.repeat(2049)])(
    'does not invent a subject for %s', (query) => expect(definitionQuestionSubject(query)).toBeNull(),
  );
  ...
  it('does not discard a tail or silently resolve alternatives', () => {
    ...
    expect(definitionQuestionSubject('Что такое икота, которой сейчас нет?')).toBe('икота, которой сейчас нет');
...
function executor(literalName?: string) {
  ...
  const row = (id: string, title: string) => ({ id, title, kind: 'term', coverage: 'definition', text_kind: 'source-excerpt', block_count: 1 });
  return {
    reads,
    async read(sql: string, args: readonly (string | number)[]) {
      reads.push({ sql, args });
      if (sql.includes("key = 'definition_reference'")) {
        return [{ value: JSON.stringify({ contract: 1, editionId: 'fixture.reference', publicationState: 'local-dev', reviewStatus: 'requires-review', identityStatus: 'source-local-proposed', linkLayout: 'numeric-v1' }) }];
      }
      if (sql.includes('n.normalized_name = ?')) {
        if (literalName && args[0] === literalName) return [row('fixture.literal', literalName)];
        if (args[0] === 'икота') return [row('fixture.first', 'Икота'), row('fixture.second', 'Икота')];
      }
      return [];
    },
  };
...
describe('question-to-reader boundary', () => {
  ...
  it('retains both source-local exact senses without a full-text scan', async () => {
    ...
    expect(sql.reads.filter((row) => row.sql.includes('n.normalized_name = ?')).map((row) => row.args[0])).toEqual(['что такое «икота»?', 'икота']);
    expect(sql.reads.some((row) => row.sql.includes('MATCH'))).toBe(false);
  });
  ...
  it('does not cut a compound description down to an attractive known term', async () => {
    ...
    expect(sql.reads.filter((row) => row.sql.includes('n.normalized_name = ?')).map((row) => row.args[0])).toEqual(['что такое икота при отсутствии глотания?', 'икота при отсутствии глотания']);
