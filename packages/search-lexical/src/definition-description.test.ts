...
function rank(query: string, texts: readonly string[]) {
  return rankDefinitionDescriptions(
    plan(query),
...
describe('description-to-definition query handler', () => {
  it.each(['как это называется', 'что за термин', 'не знаю название', '', '   '])(
    'does not manufacture a query from navigation-only input %s',
    (query) => {
  ...
  it('removes only anchored framing and preserves meaningful absence', () => {
    ...
    expect(value.terms.map((term) => term.surface)).toEqual(['запахи', 'ощущаются']);
    expect(value.terms.every((term) => term.absent)).toBe(true);
    expect(value.disjunction).not.toContain('называется');
    expect(plan('термин нарушает восприятие').terms.some((term) => term.surface === 'термин')).toBe(
      true,
    );
  });
  ...
  it('does not silently truncate overlong feature sets', () => {
    expect(
      planDefinitionDescription(Array.from({ length: 20 }, (_, i) => `word${i}`).join(' ')),
    ).toBeNull();
  });
  ...
  it('requires several defining features rather than a single common word', () => {
    expect(
      rank('движение длинного красного объекта', ['Объекты встречаются в пространстве.']),
    ).toEqual([]);
  });
  it('retains explicit absence instead of preferring its positive counterpart', () => {
    expect(
      rank('запахи не ощущаются', ['Запахи ощущаются слабее.', 'Запахи не ощущаются.']).map(
        (row) => row.id,
      ),
    ).toEqual(['fixture.1']);
  });
  it('keeps clause-local absence from leaking across punctuation', () => {
    ...
    expect(value.terms.find((term) => term.stem === 'боль')?.absent).toBe(false);
  ...
  it('is stable under candidate reordering', () => {
    ...
    expect(rankDefinitionDescriptions(value, candidates)).toEqual(
      rankDefinitionDescriptions(value, [...candidates].reverse()),
    );
  });
  it('bounds candidate count and text, including supplementary Unicode', () => {
    ...
    expect(() =>
      rankDefinitionDescriptions(value, [{ ...candidate, text: '🔬'.repeat(4097) }]),
    ).toThrow('bounded');
