...
import { createMedicationSpellingMatcher, MAX_MEDICATION_SPELLING_MATCHES } from './medication-spelling';

const names = ['Парацетамол', 'Цефтриаксон', 'Диклофенак', 'Амоксициллин', 'Азитромицин',
  'Омепразол', 'Ацикловир', 'Рисперидон', 'Левофлоксацин', 'Клозапин', 'Клоназепам',
  'Цетрин', 'Кеторол', 'Кетонал', 'Ацетилсалициловая кислота', 'Цефазолин', 'Эналаприл'];
function vocabulary(labels: readonly string[]): AliasRecord[] {
  return labels.map((name, i) => ({ id: `medicine.${i}`, alias: name, canonicalTerm: name, category: 'medication', weight: 1 }));
}
...
describe('source-vocabulary medication spelling', () => {
  ...
  it.each(['ие', 'дт', 'ео', 'зс', 'жш', 'бп', 'вф', 'гк', 'шщ', 'ий', 'еэ'])('supports both directions of pair %s without a drug-specific alias', (pair) => {
  ...
  it('does not make unrelated vowels equal by transitive phonetic folding', () => {
    expect(createMedicationSpellingMatcher(vocabulary(['аброакадабра']))('абриакадабра')[0]?.cost).toBe(3);
  });
  ...
  it('does not repair an exact non-medication alias', () => {
    const other = { id: 'finding', alias: 'Парацетомол', canonicalTerm: 'Тестовый симптом', category: 'symptom', weight: 1 };
  ...
  it('preserves the rest of the query, including strength, units and formulation', () => {
    ...
    expect(result.find((item) => item.name === 'Парацетамол')?.replacementQuery)
      .toBe('инструкция к парацетамол 500 мг таблетки');
  });
  it.each(['АД', 'мг', 'F20', '123456', 'не парацитомол', 'без парацитомола', 'аллергия на парацитомол', 'космический мармелад', 'тест\0препарат', 'а'.repeat(161), 'аmоксициллин'])('abstains on %s', (query) => {
  ...
  it('returns separate alternatives for ambiguous names, never one merged medicine', () => {
    ...
    expect(new Set(matcher('тестопрол').map((item) => item.name))).toEqual(new Set(['Тестопрал', 'Тестоприл']));
  });
  it('does not use non-medication terms to expand a medicine query', () => {
    const matcher = createMedicationSpellingMatcher([{ id: 'symptom', alias: 'Парацетамол', canonicalTerm: 'Чужой симптом', category: 'symptom', weight: 1 }]);
  ...
  it('bounds output and is deterministic under vocabulary order', () => {
    ...
    expect(createMedicationSpellingMatcher([...aliases].reverse())('пароцитамол')).toEqual(match('пароцитамол'));
  });
});
...
describe('ordinary lookup integration, not clinical inference', () => {
  ...
  it('leaves exact-name lookup unchanged', () => {
    expect(buildLookupQueryPlan('Парацетамол', aliases)).toEqual(baselineLookup('Парацетамол', aliases));
  });
  it('never shares candidates across different installed vocabularies', () => {
    expect(buildLookupQueryPlan('парацитомол', vocabulary(['Омепразол'])).analysis.warnings).toEqual([]);
