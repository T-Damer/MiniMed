...
function fixture() {
  ...
  const base = {
    initialize: vi.fn(async () => health),
    getHealth: vi.fn(async () => health),
    listDocuments: vi.fn(async () => []),
    listDocumentIdentities: vi.fn(async () => []),
    listAliases: vi.fn(async () => []),
    listEmbeddingProfiles: vi.fn(async () => []),
    close: vi.fn(async () => {}),
  } as unknown as MedicalStore;
  const forbidden = vi.fn(async () => {
    throw new Error('Whole reference corpus must not be read.');
  });
  const reference = {
    initialize: vi.fn(async () => ({ ...health, schemaVersion: 7 })),
    getHealth: forbidden,
    listDocuments: forbidden,
    listDocumentIdentities: forbidden,
    listAliases: forbidden,
    listEmbeddingProfiles: forbidden,
    close: vi.fn(async () => {}),
    reference: vi.fn(async (request: DefinitionReferenceRequest) =>
      request.op === 'status'
  ...
  const store = new MultiMedicalStore([
    { moduleId: 'core.test', store: base, required: true },
    {
      ...
      store: reference,
      definitionReference: { editionId: scope.editionId, entries: 3 },
    },
  ]);
...
describe('isolated reference capability on the existing mounted owner', () => {
  it('keeps a schema-7 reference out of schema-2 ordinary clinical projections', async () => {
    ...
    expect(await store.reference({ ...scope, op: 'search', query: 'term' })).toEqual({
      op: 'search',
      hits: [],
    });
  ...
  it('does not forward an unknown module or stale edition', async () => {
    ...
    expect(
      await store.reference({ ...scope, moduleId: 'other.test', op: 'search', query: 'term' }),
    ).toEqual({ op: 'unavailable' });
    expect(
      await store.reference({ ...scope, editionId: 'old.edition', op: 'search', query: 'term' }),
    ).toEqual({ op: 'unavailable' });
    expect(vi.mocked(read).mock.calls).toHaveLength(count);
  });
  ...
  it('rejects capability use after closing the owning store', async () => {
    const { store, reference, base } = fixture();
    await store.initialize();
    await store.close();
  ...
  it('does not relax normal clinical schema compatibility', async () => {
    ...
    const other = {
      ...
      getHealth: async () => ({
        ...
        schemaVersion: 7,
        contentPackIds: ['other'],
      }),
    };
    const store = new MultiMedicalStore([
      { moduleId: 'core', store: base },
      { moduleId: 'other', store: other },
    ]);
    await expect(store.initialize()).rejects.toThrow('Incompatible module schema');
  });
  it.each([
    { op: 'search', query: 'a', limit: 21 },
    { op: 'search', query: 'a'.repeat(2049) },
    { op: 'search', query: 'a\0b' },
    { op: 'text', id: 'entry', chunkId: 'block', offset: -1 },
    { op: 'text', id: 'entry', chunkId: 'block', offset: 0.5 },
    { op: 'status', sql: 'SELECT * FROM chunks' },
  ])('rejects unbounded or non-domain input %j', (request) => {
    expect(DefinitionReferenceRequestSchema.safeParse({ ...scope, ...request }).success).toBe(
      false,
    );
  });
});
