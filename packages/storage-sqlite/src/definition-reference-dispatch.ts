...
export function createDefinitionReferenceDispatch(sql: DefinitionReferenceSql) {
  let capability: Promise<{ reader: DefinitionReferenceReader; editionId: string; entries: number } | null> | undefined;
  const load = async () => {
    ...
    const row = rows[0];
    if (rows.length !== 1 || !row || row['schema_version'] !== 7 || typeof row['id'] !== 'string' || typeof row['manifest'] !== 'string' || row['manifest'].length > 65536) {
