...
export interface DefinitionReferenceSql {
  read(sql: string, parameters: readonly (string | number)[]): Promise<readonly Readonly<Record<string, unknown>>[]>;
}
...
function hit(row: Row, match: 'name' | 'text'): DefinitionReferenceHit {
  ...
  if (textKind !== 'editorial-paraphrase' && textKind !== 'source-gloss' && textKind !== 'source-excerpt') {
    throw new Error('Invalid reference text kind.');
  }
  return {
    id: string(row['id'], 256), title: string(row['title']), kind: string(row['kind'], 40),
    coverage: string(row['coverage'], 80), textKind, blockCount: integer(row['block_count']),
    reviewStatus: 'requires-review', identityStatus: 'source-local-proposed', match,
  };
}

/** Create a read-only capability for one already-installed, immutable content-pack handle. */
export async function createSqliteDefinitionReference(sql: DefinitionReferenceSql): Promise<DefinitionReferenceReader> {
  ...
  if (manifest['contract'] !== 1 || manifest['reviewStatus'] !== 'requires-review' ||
      manifest['publicationState'] !== 'local-dev' || manifest['identityStatus'] !== 'source-local-proposed') {
  ...
  return {
    async search(query, requested = 20) {
      if (!Number.isFinite(requested) || requested < 1 || query.length > MAX_QUERY_CHARACTERS || query.includes('\0')) return [];
      ...
      const exact = await sql.read(`SELECT ${HEADER}, MIN(CASE WHEN n.name_type = 'primary' THEN 0 ELSE 1 END) AS tier
        FROM knowledge_names n JOIN knowledge_entities e ON e.id = n.entity_id
        WHERE n.normalized_name = ? AND ${SCOPE}
        GROUP BY e.id ORDER BY tier, e.id LIMIT ?`, [normalized, ...scope, limit]);
      ...
      const body = await sql.read(`WITH matches AS MATERIALIZED (
        ...
        WHERE l.review_status = 'proposed' AND l.link_type IN ('reference:definition','reference:item')
          AND ${SCOPE}
        GROUP BY e.id ORDER BY score, e.id LIMIT ?`, [fts, ...scope, limit]);
      const ids = new Set(results.map((item) => item.id));
      for (const row of body) {
        const item = hit(row, 'text');
        if (!ids.has(item.id) && results.length < limit) { ids.add(item.id); results.push(item); }
      }
      return results;
    },
    async getCard(id) {
      identity(id);
      const rows = await sql.read(`SELECT ${HEADER} FROM knowledge_entities e WHERE e.id = ? AND ${SCOPE} LIMIT 1`, [id, ...scope]);
    ...
    async listBlocks(id, after = '') {
      ...
      const rows = await sql.read(`SELECT l.id, l.chunk_id, l.document_id,
        ...
        ORDER BY l.id LIMIT ?`, [id, after, ...scope, PAGE_BLOCKS + 1]);
      const blocks = rows.slice(0, PAGE_BLOCKS).map((row): DefinitionReferenceBlock => {
        const role = row['role'];
        if (role !== 'definition' && role !== 'item' && role !== 'context' && role !== 'annotation') throw new Error('Invalid reference block role.');
        return { linkId: string(row['id'], 300), chunkId: string(row['chunk_id'], 256),
          sourceId: string(row['document_id'], 256), role, characters: integer(row['characters']) };
      });
      return { blocks, next: rows.length > PAGE_BLOCKS ? blocks[blocks.length - 1]?.linkId ?? null : null };
    },
    async readBlock(id, chunkId, offset = 0) {
      identity(id); identity(chunkId);
      if (!Number.isSafeInteger(offset) || offset < 0 || offset > 262144) throw new Error('Invalid reference text offset.');
      const rows = await sql.read(`SELECT substr(c.original_text, ?, ?) AS body,
      ...
      return { text: body, totalCharacters: total, nextOffset: offset + BLOCK_CHARACTERS < total ? offset + BLOCK_CHARACTERS : null,
        sourceId: string(row['document_id'], 256), provenance: jsonObject(row['metadata']) };
    },
    async getSource(id) {
      identity(id);
      const rows = await sql.read(`SELECT substr(d.metadata_json, 1, 65537) AS metadata
        FROM documents d JOIN content_packs p ON p.id = d.content_pack_id
        WHERE d.id = ? AND p.id = ? AND p.enabled = 1
          AND json_extract(d.metadata_json, '$.definitionReference') = 1 LIMIT 1`, [id, editionId]);
