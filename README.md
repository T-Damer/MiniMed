...
Install a prepared icon pack with `bun run icons:install path/to/minimed-icon-pack.zip`, then run
`bun run icons:check`. `bun run icons:generate` remains available for source-master previews.
