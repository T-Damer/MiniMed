...
`bun run check` должен быть read-only в CI. Автоисправление выполняется локально отдельной командой.
...
bun run db:integrity:test
```

Если изменён поиск:

```bash
bun run benchmark:search --fixture small
```
...
1. `chore(repo): initialize Bun workspace monorepo`
...
{
  "scripts": {
    "dev": "bun run --filter @localmed/app dev",
    "dev:landing": "bun run --filter @localmed/landing dev",
    "build": "bun run --filter '@localmed/*' --if-present build",
    "format": "biome format --write .",
    "lint": "biome lint .",
    "check": "biome check .",
    "typecheck": "bun run --filter '@localmed/*' --if-present typecheck",
    "test": "bun run --filter '@localmed/*' --if-present test",
    "test:e2e": "playwright test",
    "db:migrate:test": "bun run --filter @localmed/storage-sqlite test:migrations",
    "db:integrity:test": "bun run --filter @localmed/storage-sqlite test:integrity",
    "benchmark:search": "bun run --filter @localmed/benchmarks search",
    "android:sync": "bun run --filter @localmed/app cap:sync:android",
    "android:open": "bun run --filter @localmed/app cap:open:android",
    "landing:build": "bun run --filter @localmed/landing build"
  }
}
```
