...
> Repository version: `0.6.18`
> Active target: `0.6.18` public prerelease toward `1.0`
...
- Android draws the page background beneath its transparent status bar while a static dark blur keeps
  status-bar icons legible and safe-area padding keeps controls below it. Hardware Back closes the
  active dialog or drawer, returns through nested routes and root sections, then minimizes the app at
  the search root. Capacitor Haptics supplies native light/medium/heavy impact feedback, with Web
  Vibration as the browser fallback.
