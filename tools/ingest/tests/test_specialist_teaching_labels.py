...
@pytest.mark.parametrize('title', [
    'Первое', 'Второе', 'Первая стадия', 'Вторая стадия', 'Третья стадия',
    'псевдокоронарная', 'декомпенсационная', 'аритмическая', 'смешанная',
    'обязательные', 'рекомендуемые', 'факультативные',
    'Метод исследования', 'Инвазивный метод исследования',
    'Клиническая картина заболевания', 'Важное преимущество теста',
    ...
    'Новое лечебное направление в радиологии', 'Новая эра в применении метода',
    'Основные задачи этапа стабилизации', 'Основные задача этапа стабилизации',
    'Следующим шагом для исследователей стал метод', 'Непрямым методом определения показателя',
])
...
@pytest.mark.parametrize('title', [
    'Псевдокоронарная форма миокардита', 'Первая стадия хронической болезни почек',
    'Методы диагностики уросепсиса', 'Классификация симптомов нижних мочевых путей',
    'Мальформация Киари 1 типа', 'Деперсонализация', 'ОАЭ',
    'Синдром гиперактивного мочевого пузыря', 'Ингибин', 'Фрагментация ДНК',
    'Фактор роста нервов (Nerve Growth Factor, NGF)',
    'Гуморальный иммунный ответ', 'Лёгкое', 'Новое коронавирусное заболевание',
])
...
def test_dropped_labels_keep_all_source_text_tables_and_existing_identities() -> None:
    ...
    raw = project_article(snapshot(HTML.replace('<h3>Заключение</h3>', inserted + '<h3>Заключение</h3>')))
    ...
    assert all(term['title'] not in {'Первое', 'Вторая стадия', 'обязательные', 'Метод исследования'} for term in kept)
    assert any(obj(term).get('recordRole') == 'article-overview-not-independent-term' for term in kept)
