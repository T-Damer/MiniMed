...
def test_projection_preserves_context_tables_citations_and_numeric_source() -> None:
    ...
    for term in terms[1:]:
        ...
        assert str(block['text'])[int(str(evidence['start'])):int(str(evidence['end']))] == term['title']
...
@pytest.mark.parametrize(('old', 'new'), [
    ...
    ('href="https://creativecommons.org/licenses/by-nc-sa/4.0/"', 'href="https://example.org/"'),
])
def test_capture_rejects_mismatched_identity_language_container_or_rights(old: str, new: str) -> None:
    with pytest.raises(ValueError):
        snapshot(HTML.replace(old, new))


def test_ambiguous_licenses_are_not_selected_opportunistically() -> None:
    altered = HTML.replace('</head>', '<meta name="DC.Rights" content="https://creativecommons.org/licenses/by/4.0/"></head>')
...
@pytest.mark.parametrize('value', ['Это — описание', 'В статье — три примера', 'При симптоме — другое',
                                  'Классификация:', 'Выводы', 'Одна фраза без определения'])
def test_nondefinitions_are_not_padded_into_terms(value: str) -> None:
    assert definition_label(value) is None


@pytest.mark.parametrize('value', ['Признак альфа — описание', '3.1. Признак альфа — описание',
...
def test_nested_table_requires_review_instead_of_cell_loss() -> None:
    with pytest.raises(ValueError, match='Nested'):
        source_units('<table><tr><td>Outer<table><tr><td>Inner</td></tr></table></td></tr></table>', '#tabs-2')
...
@pytest.mark.parametrize(('journal', 'aid'), [('..', '1001'), ('RFD', '1/2'), ('https:evil', '1001')])
...
def test_archive_replay_exact_and_corruption_rejected(tmp_path: Path) -> None:
    ...
    report = {'articles': [{'status': 'prepared-requires-review', 'evidence': 'article.json.gz',
                           'prepared': 'article.json', 'evidenceSha256': hashlib.sha256(archive).hexdigest(),
