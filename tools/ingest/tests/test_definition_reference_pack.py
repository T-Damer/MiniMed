...
def fixture() -> dict[str, object]:
    bodies = [
        "Точное определение тестового понятия.",
        "Первый пункт.",
        "Второй пункт.",
        "Контекст с уникальным словом контекстмаркер.",
    ]
    return {
        "version": 3,
        ...
        "sources": [
            {
                "id": 1,
                "title": "Проверочный источник",
                "baseUrl": "https://example.org/reference/",
                "sourceType": "fixture",
        ...
        "blocks": [
            {
                ...
                "text": body,
                "path": "chapter",
                "locator": f"Fixture paragraph {i + 1}",
        ...
        "terms": [
            {
                ...
                "note": "Проверочная аннотация аннотациямаркер.",
            },
            {
                "id": "fixture.second",
                "title": "Тестовый термин",
...
def test_existing_schema_shared_source_and_proposed_links(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(path) as db:
        ...
        assert db.execute(
            "SELECT DISTINCT review_status FROM knowledge_document_links"
        ).fetchall() == [("proposed",)]
        assert db.execute(
            "SELECT value FROM app_metadata WHERE key='schema_version'"
        ).fetchone() == ("6",)
        assert db.execute(
            "SELECT count(*) FROM chunks WHERE original_text=?",
            ("Точное определение тестового понятия.",),
        ).fetchone() == (1,)
        assert db.execute("SELECT sum(length(normalized_text)) FROM chunks").fetchone() == (0,)
        assert db.execute(
            "SELECT count(*) FROM sqlite_master WHERE name='definition_reference_fts_content'"
        ).fetchone() == (0,)
...
def test_editorial_and_dictionary_inputs_preserve_text_and_items(
    tmp_path: Path, version: int, text_kind: str
) -> None:
    payload = fixture()
    payload.update(
        {
            ...
            "terms": [
                {
                    ...
                    "kind": "term",
                    "aliases": ["ТТ"],
                    "definition": "Буквальный словарный текст.",
                    "items": ["Пункт один", "Пункт два"],
                    "note": "",
                    "references": [
                        {"source": 1, "path": "entry", "locator": "Exact fixture entry"}
                    ],
                }
            ],
        }
    ...
    with sqlite3.connect(path) as db:
        assert db.execute(
            "SELECT entity_id FROM knowledge_names WHERE normalized_name='в/в'"
        ).fetchone() == ("fixture.named",)
        assert (
            db.execute(
                "SELECT original_text FROM chunks WHERE original_text='Буквальный словарный текст.'"
            ).fetchone()
            is not None
        )
        assert (
            json.loads(db.execute("SELECT metadata_json FROM knowledge_entities").fetchone()[0])[
                "textKind"
            ]
            == text_kind
        )
...
@pytest.mark.parametrize(
    "path", ["../secret", "%2e%2e/secret", "https://evil.example/", "//evil.example/", "\\secret"]
)
...
def test_mentions_never_acquire_scoring_or_approval(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(path) as db:
        raw = db.execute(
            "SELECT metadata_json FROM knowledge_entities WHERE id='fixture.first'"
        ).fetchone()[0]
