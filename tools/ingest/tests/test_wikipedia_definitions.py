...
from localmed_ingest.wikipedia_definitions import (
    Snapshot, WikiApi, classify, discover, encoded, exclusion, obj, project_page, sha, source_descriptor,
)
...
def page() -> dict[str, object]:
    return {'pageid': 12, 'ns': 0, 'title': 'Тестовое понятие', 'extract': BODY,
            'revisions': [{'revid': 34, 'timestamp': '2026-09-20T00:00:00Z'}]}


def candidate() -> dict[str, object]:
    return {'pageid': 12, 'title': 'Тестовое понятие', 'families': ['general-medicine'],
...
def payload() -> dict[str, object]:
    ...
    return {'version': 3, 'reviewStatus': 'requires-review', 'publicationState': 'local-dev',
            'textKind': 'source-excerpt', 'sources': [source_descriptor('2026-09-22')],
            'blocks': [block], 'terms': [term]}
...
@pytest.mark.parametrize(('key', 'value', 'reason'), [
    ...
    ('extract', '{{Template}} Неразобранный исходный текст вместо определения.', 'unrendered-source-markup'),
    ('title', 'Иванов, Иван Иванович', 'possible-person-separate-history-queue'),
])
def test_invalid_or_other_content_is_queued_not_fabricated(key: str, value: object, reason: str) -> None:
    row = page()
    row[key] = value
...
class CategoryApi(WikiApi):
    ...
    def get(self, values: dict[str, str]) -> Snapshot:
        ...
        if category == 'Категория:Медицина' and 'cmcontinue' not in values:
            response: dict[str, object] = {'query': {'categorymembers': [
...
def test_manifest_refuses_stale_tampered_or_private_inputs(tmp_path: Path, change: str) -> None:
    ...
    if change == 'text':
        ...
        first = obj(written['inputs'][0])  # checked below by the boundary parser
        first['path'] = '../outside.json'
        manifest.write_bytes(encoded(written))
    elif change == 'duplicate':
        rows = list(written['inputs'])
...
def test_offline_cache_is_bound_to_exact_response_bytes(tmp_path: Path) -> None:
    ...
    parameters = {'action': 'query', 'format': 'json', 'formatversion': '2', 'maxlag': '5', 'pageids': '12'}
    response = json.dumps({'query': {'pages': [page()]}}, ensure_ascii=False)
    path = tmp_path / (sha(encoded(parameters)) + '.json')
    stored = {'parameters': parameters, 'response': response, 'responseSha256': sha(response.encode()), 'retrievedAt': '2026-09-22T00:00:00Z'}
