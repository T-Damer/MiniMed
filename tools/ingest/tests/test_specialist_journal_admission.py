...
@pytest.mark.parametrize(
    ...
    [
        ...
        "Ганглионевралгическая форма (zona fruste)",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "Следующий тип",
        "Большинство считает, что основной фактор",
        "Таким образом, основной механизм",
        "Естественное следствие разнообразия",
    ],
)
...
def test_all_source_paragraphs_and_metadata_survive_label_rejection() -> None:
    html = HTML.replace(
        "<h3>Заключение</h3>",
        "<p>Следующий тип — условное продолжение исходного обсуждения.</p><h3>Заключение</h3>",
    )
...
@pytest.mark.parametrize(
    ("field", "value"), [("start", -1), ("start", True), ("end", 999999), ("block", 999)]
)
