...
def test_complete_logical_tables_survive_metadata_encoding(tmp_path: Path) -> None:
    ...
    with closing(sqlite3.connect(old_path)) as old, closing(sqlite3.connect(new_path)) as new:
        ...
        assert (
            new.execute("SELECT count(*) FROM definition_reference_metadata_programs").fetchone()[0]
            > 0
        )
...
def test_json_serialization_not_just_parsed_values_is_preserved(tmp_path: Path) -> None:
    ...
    with closing(sqlite3.connect(path)) as db, db:
        ...
        for index, (identity,) in enumerate(ids):
            raw = (
                ' { "definitionReference": 1, "x": '
                + shared
                + ', "y": '
                + shared
                + ', "a": [null,true,false,1.0,3e+4,-0,"\\uD83D\\uDE00","\\u0000"],'
                + ' "unicode": "é ё", "ordinal": '
                + str(index)
                + " } "
            )
        ...
        assert db.execute(
            "SELECT count(*) FROM chunks WHERE metadata_json=?", (MARKER,)
        ).fetchone()[0] == len(ids)
...
def test_bad_view_rolls_back_all_source_mutations(tmp_path: Path) -> None:
    ...
    with closing(sqlite3.connect(path)) as db, db:
        ...
        db.execute(
            "CREATE VIEW definition_reference_chunks AS "
            "SELECT id, 'wrong' AS metadata_json FROM chunks"
        )
        with pytest.raises(ValueError, match="exact source"):
            compact_reference_metadata(db)
        assert metadata_digest(db, restored=False) == before
        assert db.execute(
            "SELECT count(*) FROM definition_reference_metadata_fragments"
        ).fetchone() == (0,)
        assert db.execute(
            "SELECT count(*) FROM definition_reference_metadata_programs"
        ).fetchone() == (0,)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"linkLayout": "unknown"},
        {"metadataLayout": "already-transformed"},
    ],
)
