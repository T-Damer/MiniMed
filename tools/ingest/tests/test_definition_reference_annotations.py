        ...
        "blocks": [
            {
                ...
                "path": "chapter",
                "locator": "Synthetic block 1",
            }
        ],
        "terms": [
            {
                "id": f"fixture.{key}.term",
                "title": f"Term {key}",
                "kind": "term",
                "aliases": [],
                "blockIds": [1],
        ...
        "etymologyStatements": [
            {
                ...
                "start": start,
                "end": end,
                "sourceStatement": body[start:end],
                "reviewStatus": "requires-review",
            }
        ],
        "historicalMentions": [
...
def test_exact_source_spans_and_no_medical_promotion(tmp_path: Path) -> None:
    ...
    with closing(sqlite3.connect(path)) as db:
        ...
        rows = db.execute(
            "SELECT a.kind, substr(c.original_text, "
            "a.start_offset + 1, a.end_offset - a.start_offset) "
        ...
        assert db.execute(
            "SELECT original_text FROM chunks WHERE original_text=?", (BODY,)
        ).fetchone() == (BODY,)


def test_same_numeric_ids_in_two_input_namespaces_do_not_cross(tmp_path: Path) -> None:
    inputs = (
        write(tmp_path, fixture("one"), "a.json"),
        write(tmp_path, fixture("two", BODY + " Other."), "b.json"),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("end", 262145),
        ("end", 0),
        ("sourceStatement", "Invented replacement"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("biography", "Invented biography"),
        ("discoveryClaim", "Invented priority"),
        ("references", []),
        ("reviewStatus", "reviewed"),
    ],
)
def test_unreviewed_name_does_not_acquire_biography(
...
def test_context_membership_is_retained_but_unrelated_card_has_no_link(tmp_path: Path) -> None:
    ...
    blocks.append(
        {
            ...
            "textSha256": digest("Other definition"),
            "path": "chapter",
            "locator": "Block 2",
        }
    )
    ...
    with closing(sqlite3.connect(path)) as db:
        assert db.execute(
            "SELECT DISTINCT entity_id FROM definition_reference_annotation_links"
        ).fetchall() == [("fixture.one.term",)]


def test_missing_membership_rejects_annotation(tmp_path: Path) -> None:
    payload = fixture()
    seq(payload["blocks"], 100).append(
        {
            "id": 2,
            "source": 1,
            "text": "Other",
            "textSha256": digest("Other"),
            "path": "chapter",
            "locator": "Block 2",
        }
    )
