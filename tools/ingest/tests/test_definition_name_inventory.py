...
from localmed_ingest.definition_name_inventory import FORMAT, add_name_inventory, read_name_manifest
...
def inventory() -> dict[str, object]:
    return {
        'format': FORMAT, 'publicationState': 'local-dev', 'reviewStatus': 'requires-review',
        'purpose': 'discovery-only', 'inputReceipts': [],
        'sources': [{'id': 1, 'title': 'Source of name only', 'baseUrl': 'https://ru.wikipedia.org/wiki/', 'releaseEligible': False}],
        'names': [{'id': 'ruwiki.definition.fixture', 'title': 'Тестовый термин', 'aliases': ['Другое имя'],
                   'kind': 'term', 'source': 1, 'path': 'Test', 'locator': 'Archived page 1', 'inputSha256': 'a' * 64}],
    }
...
def test_name_is_searchable_but_is_not_a_definition(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(dbfile) as db:
        ...
        assert db.execute("SELECT entity_id FROM knowledge_fts WHERE knowledge_fts MATCH 'Другое'").fetchall() == [('ruwiki.definition.fixture',)]
        ...
        assert db.execute("SELECT count(*) FROM definition_reference_fts WHERE definition_reference_fts MATCH 'Archived'").fetchone() == (0,)


@pytest.mark.parametrize('field,value', [
    ('definition', 'Not permitted medical prose'), ('id', 'fixture.first'),
    ('source', 999), ('path', '../outside'), ('inputSha256', 'broken'),
])
...
def test_manifest_checksum_is_required(tmp_path: Path) -> None:
    ...
    manifest = {'format': FORMAT, 'path': 'content/definition-drafts/names.json',
                'bytes': len(raw.encode()), 'sha256': digest(raw)}
...
def test_real_compacted_combined_build_keeps_definition_denominator_separate(tmp_path: Path) -> None:
    ...
    result = build_compact_definition_reference((original,), tmp_path / 'combined.db',
        input_root=tmp_path, edition_id='test.combined', version='1', built_at='2026-09-23',
        definitions_only=True, discovery_inputs=(names,))
