...
from localmed_ingest.specialist_teaching_discovery import issue_links, publisher_path, teaching_articles
...
def article(index: int = 1) -> dict[str, object]:
    ...
    return {
        'version': 3, 'id': f'journal.fixture.{index}', 'textKind': 'source-excerpt',
        'publicationState': 'local-dev', 'reviewStatus': 'requires-review',
        'sources': [{'id': 1, 'title': f'Synthetic article {index}',
                     'baseUrl': 'https://journals.eco-vector.com/RFD/',
                     'path': f'article/view/{index}/ru_RU', 'sourceType': 'specialist-journal',
                     'releaseEligible': False, 'license': 'CC-BY-4.0', 'publicationDate': '2020-01-01'}],
        'blocks': [
            {'id': 1, 'source': 1, 'text': body, 'textSha256': digest(body),
             'path': f'article/view/{index}/ru_RU', 'locator': 'p[0]'},
            {'id': 2, 'source': 1, 'text': label, 'textSha256': digest(label),
             'path': f'article/view/{index}/ru_RU', 'locator': 'p[0]; 0:15',
             'sourceSpan': {'parentBlock': 1, 'start': 0, 'end': len(label)}},
        ],
        'terms': [{'id': f'journal.fixture.{index}.term', 'title': label, 'kind': 'term',
                   'aliases': [], 'coverage': 'explicit-definition', 'blockIds': [2],
                   'detailBlocks': [1], 'labelEvidence': {'block': 2, 'start': 0, 'end': len(label)}}],
...
def test_issue_selection_respects_publisher_sections_and_title_links() -> None:
    ...
    <h2>Оригинальные исследования</h2><h3><a href="/RFD/article/view/102">Изменение показателя</a></h3>
...
@pytest.mark.parametrize('url', [
    ...
    '/RFD/article/view/0', '/other/article/view/1',
])
...
def test_source_namespace_mapping_is_reversible_and_does_not_mutate_input() -> None:
    ...
    assert obj(seq(mapped['blocks'], 10)[1])['sourceSpan'] == {'parentBlock': 40, 'start': 0, 'end': 15}
...
def test_bundles_keep_source_descriptions_once_and_rebind_local_blocks() -> None:
    ...
    assert obj(seq(payload['blocks'], 10)[3])['sourceSpan'] == {'parentBlock': 3, 'start': 0, 'end': 15}
