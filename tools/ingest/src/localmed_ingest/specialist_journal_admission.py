...
CONTEXT_START = re.compile(
    ...
    r"естественное следствие)\b",
    re.I,
)
CLAUSE = re.compile(
    r"\b(?:является|являются|существуют|нуждается|нуждаются|циркулирует|"
    r"считает|считают|считалось|имеет|имеют|регистрируется|используют|"
    r"могут|может|полагают|выделение|обусловлен|наблюдается)\b",
    re.I,
)
...
def candidate_issue(title: str) -> str | None:
    ...
    if outside.lower() in {"абортивный", "острый", "хронический", "клинически это"}:
