...
def compile_selected_definitions(
    ...
    root = obj(payload)
    if set(root) != {"format", "id", "inspectedAt", "sources", "entries"} or root["format"] != FORMAT:
    ...
    for value in rows:
        row = obj(value)
        if set(row) != {
            "id", "title", "kind", "aliases", "source", "path", "definition", "locator",
            "authors", "reviewers", "sourceUpdated", "classification", "population",
        }:
    ...
    deferred = [
        {"id": row["id"], "title": row["title"], "reason": "existing-definition-name-needs-sense-review"}
        for row in validated if normalized_name(str(row["title"])) in existing
    ]
    ...
    for row in selected:
        ...
        blocks.append({
            "id": block_id, "source": row["source"], "text": body,
            "textSha256": digest(body), "path": row["path"], "locator": row["locator"],
            "authors": row["authors"], "reviewers": row["reviewers"],
            "sourceUpdated": row["sourceUpdated"], "classification": row["classification"],
            "population": row["population"], "inspectedAt": inspected_at,
            "acquisitionMethod": "inspected-web-excerpt",
            "fullPageSnapshotAvailable": False, "medicalReview": "requires-review",
        })
        terms.append({
            "id": row["id"], "title": row["title"], "kind": row["kind"],
            "aliases": row["aliases"], "coverage": "definition", "blockIds": [block_id],
        })
    used_sources = {number(row["source"]) for row in selected}
    catalog: dict[str, object] = {
        "version": 3, "id": catalog_id, "reviewStatus": "requires-review",
    ...
    report: dict[str, object] = {
        "authoringSha256": digest(encoded(root)), "candidates": len(validated),
        "selectedDefinitions": len(selected), "selectedIds": [row["id"] for row in selected],
        "deferred": deferred, "sourcePagesInspected": len(page_words),
        "maximumWordsPerPage": max(page_words.values()), "newInstruments": 0,
        "boundary": (
