...
from .specialist_journal_reference import HOST, MAX_PAGE_BYTES, article_url, capture_page, project_article
...
def read_original(url: str) -> bytes:
    ...
    request = urllib.request.Request(url, headers={
        'User-Agent': USER_AGENT, 'Accept-Language': 'ru', 'Accept-Encoding': 'identity',
    })
...
def collect(selection: Path, output: Path) -> dict[str, object]:
    ...
    if selected.get('version') != 1 or selected.get('sourcePolicy') != 'specialist-medical-sources-2026-09-22':
    ...
    with tempfile.TemporaryDirectory(prefix='specialist-sources-', dir=output.parent) as temporary:
        ...
        for candidate in candidates:
            ...
            record: dict[str, object] = {'journal': journal, 'articleId': aid, 'url': url, 'selectionFocus': candidate.get('focus')}
            ...
            try:
                ...
                (staged / evidence).write_bytes(gzip.compress(evidence_raw, compresslevel=9, mtime=0))
                ...
                counts.update({'records': len(terms), 'articles': 1, 'blocks': len(blocks),
                ...
                outcomes.append({**record, 'status': 'prepared-requires-review',
                                 'title': sources[0]['title'], 'publicationDate': sources[0]['publicationDate'],
                                 'license': sources[0]['license'], 'articleType': sources[0]['articleType'],
                                 'records': len(terms), 'blocks': len(blocks),
                                 'evidence': evidence, 'evidenceSha256': hashlib.sha256((staged / evidence).read_bytes()).hexdigest(),
                                 'prepared': prepared, 'preparedSha256': hashlib.sha256(prepared_raw).hexdigest(),
                                 'proposedTitles': [t['title'] for t in terms], 'extraction': projected['extraction']})
            except (ValueError, UnicodeError, OSError, urllib.error.URLError) as error:
                outcomes.append({**record, 'status': 'pending-source-review',
                                 'reason': type(error).__name__ + ': ' + str(error)[:400]})
        report = {'version': 1, 'selectionSha256': hashlib.sha256(raw_selection).hexdigest(),
                  'robotsSha256': hashlib.sha256(robot_raw).hexdigest(), 'selectedArticles': len(candidates),
                  'counts': dict(counts), 'articles': outcomes,
...
def replay(collection: Path) -> dict[str, object]:
    ...
    for value in seq(report.get('articles'), 24):
        ...
        if hashlib.sha256(evidence.read_bytes()).hexdigest() != row.get('evidenceSha256') or hashlib.sha256(prepared.read_bytes()).hexdigest() != row.get('preparedSha256'):
    ...
    return {'articlesReplayed': compared, 'sourceBlocksReplayed': blocks, 'networkRequests': 0,
            'exactPreparedReplay': True, 'scope': 'Deterministic source fidelity, not clinical review.'}
