...
class TopicParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        values = dict(attrs)
        hidden = (any(row[1] for row in self.stack) or tag in {"script", "style", "noscript"}
...
def parse_topic(raw: bytes, requested: str, retrieved_at: str) -> dict[str, object]:
    ...
    return {"title": title, "url": canonical, "topicId": topic_id,
            "definition": sentence, "revisionLabel": parser.text("revision")[:512],
            "authorIds": parser.metadata.get("og:author"),
            "sourceVerification": {"method": "visible-paragraph-exact-v1",
                "responseSha256": checksum(raw), "paragraphSha256": checksum(paragraph),
                "excerptSha256": checksum(sentence) if sentence else None,
                "start": 0, "end": len(sentence) if sentence else None,
                "retrievedAt": retrieved_at, "normalization": "HTML entities and whitespace only"}}
    ...
    for url in schedule:
        try:
            ...
            if exc.code in {401, 403, 429}:
                raise RuntimeError("Source access/rate limit requires stopping, not bypassing") from exc
