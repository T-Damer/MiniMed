...
MAX_TEXT = 262144
KINDS = frozenset({"term", "symptom", "syndrome", "criterion_set", "scale", "classification", "law", "tool", "history_note"})
COVERAGE = frozenset({"definition", "explicit-definition", "definition-section", "contextual-definition", "criterion-list", "classification", "section-overview", "section-excerpt", "tool-description", "source-description", "cross-reference", "mention-only", "gloss"})
...
def source_path(source: dict[str, object], value: object) -> str:
    ...
    if (parsed.scheme != "https" or not parsed.netloc or parsed.username or parsed.password
            ...
            or resolved.netloc != parsed.netloc or resolved.scheme != parsed.scheme
            or not resolved.path.startswith(parsed.path) or decoded.startswith("/")
...
class Projection:
    ...
    def block(self, source: Source, body: str, provenance: dict[str, object]) -> str:
        ...
        if chunk_id not in self.chunks:
            chunk = PackChunk(
                id=chunk_id, order_index=len(source.chunks), original_text=body,
                normalized_text="", anchor=f"reference/{identity}",
    ...
    def add(self, payload: object, receipt: str) -> None:
        ...
        if (root.get("reviewStatus") != "requires-review" or root.get("publicationState") != "local-dev"
        ...
        for value in seq(root.get("sources"), 1000):
            ...
            if descriptor.get("sourceType") == "owner-pdf":
                ...
                if "/" in file_name or "\\" in file_name or not re.fullmatch(r"[a-f0-9]{64}", text(descriptor.get("sourceSha256"), 64)):
                    raise ValueError("Invalid owner source provenance")
            # Module-local numbers are not global identity. Exact descriptors can be shared across shards.
            descriptor.pop("id")
        ...
        if version == 3:
            for value in seq(root.get("blocks"), 100000):
                ...
                path = source_path(source.descriptor, row.get("path", source.descriptor.get("path", "")))
                locator = text(row.get("locator"), 4096)
                provenance = {key: val for key, val in row.items() if key not in {"id", "source", "text"}}
        ...
        for value in seq(root.get("terms"), 50000):
            ...
            if not re.fullmatch(r"[a-z0-9]+(?:[.-][a-z0-9]+)*", entry_id) or entry_id in self.entries:
            ...
            if version == 3:
                ...
                for key, role in (("blockIds", "definition"), ("itemBlocks", "item"), ("detailBlocks", "context")):
                ...
                for item in seq(row.get("references"), 8):
                    ...
                    references.append({**ref, "source": source.id,
                ...
                primary = next(source for source in local_sources.values() if source.id == primary_id)
                body = text(row.get("definition"), MAX_TEXT)
                links.append(("definition", self.block(primary, body, {"citations": references, "textSha256": digest(body)})))
                for item in seq(row.get("items", []), 1000):
                    body = text(item, MAX_TEXT)
                    links.append(("item", self.block(primary, body, {"citations": references, "textSha256": digest(body)})))
            ...
            extra = {key: val for key, val in row.items() if key not in {
                "id", "title", "kind", "aliases", "definition", "items", "blockIds", "itemBlocks", "detailBlocks", "references", "coverage", "definitionKind",
            }}
            if extra:
                source, _ = self.chunks[links[0][1]]
                annotation = encoded(extra)
                text(annotation, MAX_TEXT)
                links.append(("annotation", self.block(source, annotation, {"format": "source-metadata-json", "inputSha256": receipt})))
            self.entries[entry_id] = Entry(entry_id, title, kind, names, coverage, TEXT_KINDS[version], links, receipt)
        annotations = {key: val for key, val in root.items() if key not in {
            "sources", "blocks", "terms", "version", "id", "textKind", "publicationState", "reviewStatus",
        }}
        self.annotations[receipt] = annotations
        if len(self.entries) > 100000:
            raise ValueError("Reference build entry budget exceeded")

    def build(self, output: Path, *, edition_id: str, version: str, built_at: str) -> dict[str, object]:
        if not self.entries or output.exists():
            raise ValueError("Need nonempty inputs and a new immutable output path")
        manifest = {"contract": 1, "editionId": edition_id, "version": version,
                    "reviewStatus": "requires-review", "publicationState": "local-dev",
                    "identityStatus": "source-local-proposed", "receipts": self.receipts,
                    "entries": len(self.entries), "sources": len(self.sources), "blocks": len(self.chunks)}
        documents: list[PackDocument] = []
        for source in sorted(self.sources.values(), key=lambda item: item.id):
            documents.append(PackDocument(
                id=source.id, title=text(source.descriptor["title"]), source_type="medical_reference",
                status="draft", specialties=[], metadata={"definitionReference": 1, "source": source.descriptor,
                "releaseEligible": False, "reviewStatus": "requires-review"},
                version=PackVersion(id=source.id + "@" + version, label=version,
                                    source_checksum="sha256:" + digest(encoded(source.descriptor)), extracted_at=built_at),
                sections=[PackSection(id=source.id + ".blocks", title="Исходные фрагменты",
                          normalized_title="исходные фрагменты", section_type="reference-source", depth=1,
                          order_index=0, anchor=source.id + "/blocks", section_path=["Исходные фрагменты"],
                          chunks=list(source.chunks.values()))],
            ))
        pack = ContentPack(manifest=PackManifest(id=edition_id, version=version, schema_version=6,
                    title="MiniMed definition reference (requires review)", built_at=built_at,
                    checksum="sha256:" + digest(encoded(manifest)), publication_state="local-dev"), documents=documents)
        output.parent.mkdir(parents=True, exist_ok=True)
        with tempfile.TemporaryDirectory(prefix="definition-build-", dir=output.parent) as directory:
            ...
            with sqlite3.connect(staged) as database:
                ...
                database.execute("INSERT INTO app_metadata VALUES (?, ?)", ("definition_reference", encoded(manifest)))
                for receipt, annotations in self.annotations.items():
                    database.execute("INSERT INTO app_metadata VALUES (?, ?)", ("definition_reference_annotations:" + receipt, encoded(annotations)))
                for entry in sorted(self.entries.values(), key=lambda item: item.id):
                    metadata = {"definitionReference": 1, "sourceEntryId": entry.id, "coverage": entry.coverage,
                                "textKind": entry.text_kind, "reviewStatus": "requires-review",
                                "identityStatus": "source-local-proposed", "inputSha256": entry.receipt,
                                "blockCount": len(entry.links), "editionId": edition_id}
                    database.execute("INSERT INTO knowledge_entities VALUES (?, ?, ?, ?, ?, ?)",
                                     (entry.id, entry.kind, entry.title, normalized_name(entry.title), "{}", encoded(metadata)))
                    for index, name in enumerate(entry.names):
                        database.execute("INSERT INTO knowledge_names VALUES (?, ?, ?, ?, ?, ?, ?)",
                                         (f"{entry.id}.name.{index}", entry.id, name, normalized_name(name), "ru",
                                          "primary" if index == 0 else "source-alias", 2.0 if index == 0 else 1.0))
                    for index, (role, chunk_id) in enumerate(entry.links):
                        source, chunk = self.chunks[chunk_id]
                        database.execute("INSERT INTO knowledge_document_links VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                            (f"{entry.id}.reference.{index:06d}", entry.id, source.id, source.id + "@" + version,
                             source.id + ".blocks", chunk.id, "reference:" + role, 1.0, "proposed",
                ...
                database.execute("INSERT INTO definition_reference_fts(definition_reference_fts) VALUES ('rebuild')")
                database.execute("INSERT INTO definition_reference_fts(definition_reference_fts, rank) VALUES ('integrity-check', 1)")
        ...
        return {**manifest, "sqliteBytes": output.stat().st_size, "sqliteSha256": hashlib.sha256(output.read_bytes()).hexdigest(),
                "integrity": integrity, "foreignKeyViolations": foreign_keys,
                "boundaries": "Source-local draft navigation only. No approved knowledge facts/relations, app lifecycle or Android qualification."}


def build_definition_reference(inputs: tuple[Path, ...], output: Path, *, input_root: Path,
                               edition_id: str, version: str, built_at: str) -> dict[str, object]:
...
def main() -> None:
    ...
    report = build_definition_reference(tuple(args.input), args.output, input_root=args.input_root,
