...
def recover_names(root: Path, archive: Path) -> tuple[dict[str, object], dict[str, object]]:
    ...
    for path in inputs:
        ...
        for item in seq(payload['terms'], 50000):
            ...
            names.append({
                'id': identifier, 'title': title, 'kind': text(row['kind'], 40),
    ...
    inventory = {'format': FORMAT, 'publicationState': 'local-dev',
                 'reviewStatus': 'requires-review', 'purpose': 'discovery-only',
                 'sources': sources, 'names': names, 'inputReceipts': receipts}
    report = {'sourceLocalNames': len(names), 'normalizedTitles': len(titles),
              'sourceInputs': len(receipts), 'sources': len(sources),
...
def add_name_inventory(projection: Projection, inventory: object, receipt: str) -> int:
    from .definition_reference_pack import (
        KINDS, Entry, Source, digest, encoded, number, obj, seq, source_path, strings, text,
    )
    root = obj(inventory)
    if (root.get('format') != FORMAT or root.get('purpose') != 'discovery-only'
    ...
    if set(root) - {'format', 'purpose', 'publicationState', 'reviewStatus', 'sources', 'names', 'inputReceipts'}:
    ...
    for value in seq(root.get('sources'), 1000):
        ...
        key = digest(encoded(descriptor))
        source = projection.sources.get(key) or staged_sources.setdefault(key, Source('reference.source.' + key, descriptor))
        local[local_id] = source
    staged: list[tuple[dict[str, object], Source]] = []
    seen: set[str] = set()
    for value in seq(root.get('names'), 50000):
        row = obj(value)
        if set(row) != {'id', 'title', 'kind', 'aliases', 'source', 'path', 'locator', 'inputSha256'}:
            raise ValueError('Name record contains missing fields or medical prose')
        identifier = text(row['id'], 256)
        if not re.fullmatch(r'[a-z0-9]+(?:[.-][a-z0-9]+)*', identifier) or identifier in seen or identifier in projection.entries:
    ...
    for row, source in staged:
        ...
        origin = {'discoveryOnly': True, 'path': row['path'], 'locator': row['locator'],
                  'originalInputSha256': row['inputSha256'], 'definitionStatus': 'needs-definition'}
        chunk_id = projection.block(source, encoded(origin), {'format': 'name-discovery-json', **origin})
        identifier, title = str(row['id']), str(row['title'])
        projection.entries[identifier] = Entry(identifier, title, str(row['kind']),
            list(dict.fromkeys([title, *strings(row['aliases'])])), 'needs-definition',
            'source-excerpt', [('annotation', chunk_id)], receipt)
    return len(staged)
...
def read_name_manifest(root: Path) -> tuple[Path, ...]:
    ...
    if len(raw) > MAX_INPUT_BYTES or len(raw) != row['bytes'] or hashlib.sha256(raw).hexdigest() != row['sha256']:
