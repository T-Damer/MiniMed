...
GENERIC_HEADINGS = re.compile(
    ...
    r'вклад авторов|дополнительная информация|благодарност|references|abstract)', re.I,
)
CLINICAL_HEADING = re.compile(
    ...
    r'феномен|бред|соматоформ|диссоциатив', re.I,
)
...
BAD_START = re.compile(
    ...
    r'результат|данный|данная|данное|данные|основу|один|одним)\b', re.I,
)
...
def article_url(journal: str, article_id: str) -> str:
    if not re.fullmatch(r'[A-Za-z0-9-]{1,40}', journal) or not re.fullmatch(r'[1-9]\d{0,11}', article_id):
...
def capture_page(raw: bytes, journal: str, article_id: str, accessed: str) -> dict[str, object]:
    ...
    license_urls = {
        ...
        for v in meta.get('DC.Rights', []) if 'creativecommons.org/licenses/' in v
    }
    ...
    if label is None or label.get_text(' ', strip=True).lower() not in {'полный текст', 'full text'}:
    ...
    fragment = str(body[0])
    return {
        'version': 1, 'journal': journal, 'articleId': article_id,
        'url': article_url(journal, article_id), 'accessed': accessed,
        'responseSha256': __import__('hashlib').sha256(raw).hexdigest(),
        'metadata': meta, 'licenseUrl': license_url,
        'fullTextHtml': fragment, 'fullTextHtmlSha256': digest(fragment),
        'bibliographyHtml': bibliography, 'bibliographyHtmlSha256': digest(bibliography),
...
def source_units(fragment: str, container: str) -> tuple[list[Unit], dict[str, int]]:
    ...
    def walk(node: Element | str, locator: str) -> None:
        if isinstance(node, str):
            if node.strip():
                units.append(Unit(node.strip(), locator, tuple(v for _, v in headings), 'text', [], []))
            return
        if omitted(node):
        ...
        if node.tag in atomic:
            ...
            units.append(Unit(body, locator, tuple(v for _, v in headings), node.tag, _geometry(node), proposals))
            return
...
def proposed_kind(label: str) -> str:
    value = label.lower()
    for needle, kind in (('классификац', 'classification'), ('критери', 'criterion_set'),
                         ('шкал', 'scale'), ('синдром', 'syndrome'), ('симптом', 'symptom')):
        if needle in value:
            return kind
    return 'term'
...
def project_article(snapshot: object, source_number: int = 1) -> dict[str, object]:
    ...
    declared = {v.replace('http://', 'https://').rstrip('/') + '/' for v in values('DC.Rights') if 'creativecommons.org/licenses/' in v}
    ...
    full, bib = text(snap.get('fullTextHtml'), 4 * 1024 * 1024), text(snap.get('bibliographyHtml'), 2 * 1024 * 1024, empty=True)
    if digest(full) != snap.get('fullTextHtmlSha256') or digest(bib) != snap.get('bibliographyHtmlSha256'):
    ...
    if len(publication) != 1 or not re.fullmatch(r'\d{4}-\d{2}-\d{2}', publication[0]) or len(doi) != 1:
    ...
    source = {
        'id': source_number, 'title': title, 'baseUrl': HOST + '/' + journal + '/',
        'path': 'article/view/' + aid + '/ru_RU', 'sourceType': 'specialist-journal',
        'authority': 'authored-specialist-publication', 'releaseEligible': False,
        'journal': _russian(values('citation_journal_title'), 'journal title'),
        'authors': values('citation_author'), 'publicationDate': publication[0],
        'articleType': values('DC.Type.articleType'), 'doi': doi[0],
        'volume': values('citation_volume'), 'issue': values('citation_issue'),
        'pages': values('DC.Identifier.pageNumber'), 'accessed': text(snap.get('accessed'), 40),
        'license': LICENSES[license_url], 'licenseUrl': license_url,
        'nonCommercialOnly': 'by-nc-' in license_url,
        'copyright': values('DC.Rights'), 'sourceUrl': url,
        'responseSha256': snap['responseSha256'], 'articleFragmentSha256': digest(full),
    ...
    for number, unit in enumerate(all_units, 1):
        blocks.append({
            'id': number, 'source': source_number, 'text': unit.body, 'textSha256': digest(unit.body),
            'path': source['path'], 'locator': unit.locator, 'sectionPath': list(unit.headings),
            'sourceElement': unit.tag, 'tables': unit.tables,
    ...
    records: list[dict[str, object]] = [{
        'id': stem, 'title': title, 'kind': 'term', 'aliases': [],
        'coverage': 'source-description', 'blockIds': [first],
    ...
    for index, unit in enumerate(body):
        ...
        if unit.tag == 'heading' and len(unit.body) <= 180 and not GENERIC_HEADINGS.search(unit.body) and CLINICAL_HEADING.search(unit.body):
        ...
        for label, evidence in labels:
            ...
            related = [i + 1 for i, other in enumerate(body)
            ...
            records.append({
                ...
                'title': label, 'kind': proposed_kind(label), 'aliases': aliases,
                'coverage': 'section-excerpt' if unit.tag == 'heading' else 'explicit-definition',
                'blockIds': [number], 'detailBlocks': related,
                'labelEvidence': {'block': number, 'start': start, 'end': start + len(label)},
                'proposalMethod': 'source-heading' if unit.tag == 'heading' else 'source-definition-clause',
                'articleRecordId': stem,
            })
    return {
        'version': 3, 'id': stem + '.input', 'publicationState': 'local-dev',
        'reviewStatus': 'requires-review', 'textKind': 'source-excerpt',
        'sources': [source], 'blocks': blocks, 'terms': records,
        'extraction': {'bodyBlocks': len(body), 'bibliographyBlocks': len(bibliography),
                       'omissions': omissions, 'bibliographyOmissions': bib_omissions,
                       'clinicalReview': 'not-performed', 'instrumentScoring': 'not-created'},
    }
