...
def compile_msd_names(
    ...
    catalog = obj(payload)
    if catalog.get("format") != FORMAT or catalog.get("scope") != "ru/professional public topic navigation":
        raise ValueError("Unsupported MSD topic metadata")
    receipts = seq(catalog.get("receipts"), 70)
    known_receipts = {
        text(obj(row).get("sha256"), 64): text(obj(row).get("path"), 4096)
        for row in receipts
    }
    ...
    for value in seq(catalog.get("references"), 20000):
        row = obj(value)
        if set(row) != {
            "id", "title", "sourceTitle", "path", "sectionPath", "chapterTitle",
            "chapterPath", "indexSha256",
        }:
        ...
        if not re.fullmatch(r"[a-f0-9]{64}", receipt) or known_receipts.get(receipt) != row["sectionPath"]:
    ...
    for identifier, row in sorted(unique.items()):
        ...
        reason = (
            "overview-or-navigation-title" if OVERVIEW.search(title) else
            "already-searchable-name-needs-sense-review" if normalized in existing else
            "same-new-title-needs-sense-review" if normalized in added_names else None
        )
        if reason:
        ...
        selected.append({
            "id": identifier, "title": title, "kind": "term", "aliases": [], "source": 1,
            "path": str(row["path"]).lstrip("/"),
            "locator": "Название темы в оглавлении: " + str(row["sectionPath"]) +
                       " → " + str(row["chapterTitle"]),
            "inputSha256": row["indexSha256"],
        })
    result: dict[str, object] = {
        "format": NAME_FORMAT, "publicationState": "local-dev", "reviewStatus": "requires-review",
        "purpose": "discovery-only",
        "sources": [{
            "id": 1, "title": "Справочник MSD — оглавление профессиональной русской версии",
            "baseUrl": HOST + "/", "sourceType": "medical-topic-index",
            "authority": "third-party", "releaseEligible": False,
            "rightsStatus": "metadata-only-content-reuse-not-cleared", "language": "ru",
            "contentScope": "Названия и адреса тем; медицинские тексты не импортированы",
        }],
        "names": selected, "inputReceipts": receipts,
    }
    return result, {
        "sourceTopics": len(unique), "newSearchableNames": len(selected),
        "deferred": deferred, "medicalDefinitionsImported": 0,
        "boundary": (
