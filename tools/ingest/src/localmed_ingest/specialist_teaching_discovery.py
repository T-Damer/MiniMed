...
TEACHING = re.compile(
    ...
    r'lecture|review|clinical.{0,20}guideline|continuing medical', re.I,
)
EXCLUDED = re.compile(
    r'некролог|юбиле|поздравлен|к памяти|рецензи|редакционн|реклам|'
    r'сравнительн.{0,30}эффективност|randomi[sz]ed|obituar|anniversary|editorial', re.I,
)
FOCUS = re.compile(
    r'синдром|симптом|семиотик|термин|определени|классификац|критери|'
    r'дифференциальн|диагност|патогенез|syndrome|classification|diagnos|criteria', re.I,
)
...
def publisher_path(href: str, journal: str, kind: str) -> str | None:
    ...
    if (value.scheme != 'https' or value.netloc != 'journals.eco-vector.com'
            or value.query or value.fragment or not match):
        return None
    return match[1]
...
def teaching_articles(raw: bytes, journal: str, issue: str) -> list[dict[str, object]]:
    ...
    for node in _html(raw).find_all(['h2', 'h3', 'h4', 'a']):
        ...
        if (not aid or aid in selected or not 8 <= len(title) <= 600
                or EXCLUDED.search(title) or not re.search(r'[А-Яа-яЁёA-Za-z]', title)):
            continue
        # Never treat an abstract, author name, download link or arbitrary research item as a lecture.
        if not TEACHING.search(section) and not re.search(r'лекци|lecture', title, re.I):
            continue
        selected[aid] = {
            'journal': journal, 'articleId': aid, 'title': title,
            ...
            'discovery': {
                ...
                'section': section, 'issueResponseSha256': hashlib.sha256(raw).hexdigest(),
            },
        }
    return list(selected.values())


def discover(root: Path, output: Path, issues_per_journal: int = 12,
             articles_per_journal: int = 18) -> dict[str, object]:
    ...
    for journal in JOURNALS:
        try:
            ...
            outcomes.append({'journal': journal, 'status': 'archive-pending',
        ...
        for issue in issues[:issues_per_journal]:
            try:
                ...
                outcomes.append({'journal': journal, 'issue': issue, 'status': 'indexed',
                                 'candidates': len(found), 'responseSha256': hashlib.sha256(raw).hexdigest()})
            except (ValueError, UnicodeError, OSError, urllib.error.URLError) as error:
                outcomes.append({'journal': journal, 'issue': issue, 'status': 'issue-pending',
                                 'reason': type(error).__name__ + ': ' + str(error)[:200]})
        ordered = sorted(pool.values(), key=lambda row: (
            not bool(FOCUS.search(text(row['title']))), text(row['articleId']),
        ))
        candidates.extend(ordered[:articles_per_journal])
    selections: list[str] = []
    for start in range(0, len(candidates), 24):
        name = f'selection-{start // 24 + 1:02d}.json'
        (output / name).write_text(encoded({
            'version': 1, 'sourcePolicy': 'specialist-medical-sources-2026-09-22',
            'articles': candidates[start:start + 24],
        }) + '\n', encoding='utf-8')
        selections.append(name)
    report = {
        'version': 1, 'journals': list(JOURNALS), 'issuesPerJournalLimit': issues_per_journal,
        'articlesPerJournalLimit': articles_per_journal, 'selectedCandidates': len(candidates),
        'alreadyActiveObservationsSkipped': excluded_existing,
        'robotsSha256': hashlib.sha256(robots_raw).hexdigest(),
        'selections': selections, 'issues': outcomes,
        'boundary': 'Bounded issue-index discovery only; not full journal coverage, clinical review or licensed text acquisition.',
    }
