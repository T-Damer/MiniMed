    ...
    with tempfile.TemporaryDirectory(prefix="journal-intake-", dir=output.parent) as temporary:
            ...
            counts.update(
                {
                    ...
                    "blocks": len(blocks),
                    "tables": sum(len(seq(b["tables"], 1000)) for b in blocks),
                }
            )
            outcomes.append(
                {
                    "status": "selected-requires-clinical-review",
                    ...
                    "nonCommercialOnly": source["nonCommercialOnly"],
                    "evidence": row["evidence"],
                    "evidenceSha256": row["evidenceSha256"],
                    "prepared": filename,
                    "preparedSha256": hashlib.sha256(encoded_payload).hexdigest(),
                    "records": len(terms),
                    "names": sorted(names),
                    "distinctNormalizedNames": len({normalized_name(name) for name in names}),
                    "previousCandidateNamesNotSelected": [
                        name for name in before if name not in names
                    ],
                    "blocks": len(blocks),
        ...
        result = {
            ...
            "boundary": (
                ...
                "Article overviews and section cards are counted separately "
                "from definition clauses. "
                "Old source classification and terminology are retained, not harmonized."
            ),
        }
        (staged / "intake-report.json").write_text(
