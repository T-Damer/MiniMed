...
def remap_article(
    payload: object, sources: dict[int, int], blocks: dict[int, int]
) -> dict[str, object]:
...
def bundle_articles(articles: list[dict[str, object]], stem: str) -> list[dict[str, object]]:
    ...
    for payload in articles:
        ...
        if (
            ...
            or not block_rows
            or not term_rows
            or any(row.get("sourceType") != "specialist-journal" for row in source_rows)
        ):
        ...
        if len(set(local_sources)) != len(local_sources) or len(set(local_blocks)) != len(
            local_blocks
        ):
        ...
        for attempt in range(2):
            ...
            restored = remap_article(
                remapped, {v: k for k, v in smap.items()}, {v: k for k, v in bmap.items()}
            )
            if restored != payload:
...
def activate_bundles(root: Path, selected: Path, stem: str) -> dict[str, object]:
    ...
    report["refresh"] = {
        ...
        "bundleStem": stem,
        "bundleCount": len(payloads),
        "numericRoundTripExact": True,
    }
