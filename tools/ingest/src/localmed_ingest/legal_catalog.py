...
def _response_items(payload: object) -> tuple[list[dict[str, object]], int]:
    ...
    pages = payload.get("pagesTotalCount", 1)
    ...
    for index, item in enumerate(items):
        ...
        rows.append({str(key): cast(object, value) for key, value in item.items()})
...
def _authorities(detail: dict[str, object] | None) -> list[str]:
    ...
    for authority in value:
        if not isinstance(authority, dict):
            continue
        name = _clean(authority.get("name"))
...
def _document_type(detail: dict[str, object] | None) -> str | None:
    if detail is None:
        return None
    value = detail.get("documentType")
    return _clean(value.get("name")) if isinstance(value, dict) else None
    ...
    *,
    ...
    for query in config.queries:
        ...
        while page <= min(total_pages, query.max_pages):
            ...
            raw_pages.append({"queryId": query.id, "page": page, "url": url, "payload": payload})
            for item in items:
                ...
                records_by_eo[eo_number] = LegalCatalogRecord(
                    ...
                    raw_metadata={**item, **({"detail": detail} if detail else {})},
                )
            page += 1
    ...
    ledger = LegalCatalogLedger(
        ...
        raw_pages_checksum=_sha256_json(raw_pages),
        records=records,
        modules=modules,
        summary=LegalCatalogSummary(
