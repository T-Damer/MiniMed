...
const aliases = db
  .query("SELECT alias, canonical_term FROM aliases WHERE category = 'medication'")
  .all();
...
for (const [i, query] of userQueries.entries())
  cases.push({
    id: `user.${i}`,
    family: 'user-kanephron',
    query,
    expected: byName.get('канефрон н') ?? [],
    sourceName: 'Канефрон Н',
  });
const marked = [...byName]
  .filter(([name, ids]) => /^[а-я]{7,48}[ -][а-я]$/u.test(name) && ids.length)
...
for (const [i, item] of marked.slice(0, 20).entries()) {
  ...
  for (const [family, query] of Object.entries(mutations)) {
    if (query === stem || byName.has(query) || byTitle.has(query)) {
      exactCollisionsExcluded += 1;
      continue;
    }
    cases.push({
      id: `marked.${i}.${family}`,
...
assert(
  cases.every((row) => row.expected.length),
  'Do not hide missing corpus targets',
);
const exact = [
  ...new Set([
...
async function sample(corePath) {
  ...
  try {
    ...
    const filterCases = [
      ...
      { sectionTypes: ['missing-section'] },
      { specialties: ['missing-specialty'] },
      { ageGroups: ['missing-age'] },
    ];
    for (const filters of filterCases)
      assert.equal((await search('канефрно', filters)).groups.length, 0);
    const analyses = [];
...
const exactChanges = exact.filter(
  (query) =>
    JSON.stringify(before.exactResults[query]) !== JSON.stringify(after.exactResults[query]),
);
...
const report = {
  baselineCommit: '4c66e84f31632f3847c735a300ce15b1ca596019',
  checkedCommit,
  databaseSha256,
  sameDatabaseForBoth: databaseSha256 === hash(readFileSync(dbPath)),
  caseSha256: hash(JSON.stringify(cases)),
  markedSourceNames: marked.length,
  exactCollisionsExcluded,
  byFamily: Object.fromEntries(
    [...new Set(cases.map((row) => row.family))].map((family) => [
      family,
      { before: summary(before.outcomes, family), after: summary(after.outcomes, family) },
    ]),
  ),
  exactChecked: exact.length,
  exactChanges,
  regressions,
  contextChecks: after.contextChecks,
  filterChecks: after.filterChecks,
...
assert(
  after.outcomes
    .filter((row) => row.family === 'user-kanephron')
    .every((row) => row.rank !== null && row.rank <= 5),
);
