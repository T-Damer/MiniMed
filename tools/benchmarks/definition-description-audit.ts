...
if (!databasePath || !baselinePath || !outputPath) throw new Error('Usage: bun tools/benchmarks/definition-description-audit.ts DB BASELINE_READER REPORT');
...
const reset = () => { rowsReturned = 0; bytesReturned = 0; reverseQueries = 0; };
async function sample(reader: DefinitionReferenceReader, query: string, expected: string) {
  ...
  return { rank: index < 0 ? null : index + 1, ids: hits.map((hit) => hit.id), latencyMs,
    rowsReturned, bytesReturned, reverseQueries };
}
const outcomes = [];
for (const item of dataset.cases) {
  ...
  outcomes.push({ ...item, corpusMissing: false,
...
for (const query of dataset.negativeControls) {
  negatives.push({ query, before: (await before.search(query, 20)).map((hit) => hit.id),
...
const count = database.query("SELECT json_extract(metadata_json,'$.coverage') AS coverage,count(*) AS records FROM knowledge_entities GROUP BY coverage").all();
...
for (const family of [...new Set(dataset.cases.map((item) => item.family))]) {
  ...
  byFamily[family] = { total: eligible.length,
...
const stats = (key: 'before' | 'after') => {
  const latencies = eligible.flatMap((row) => row[key] ? [row[key].latencyMs] : []).sort((a, b) => a - b);
  return { top1: eligible.filter((row) => row[key]?.rank === 1).length,
...
const report = {
  ...
  sameDatabaseForBoth: true, cases: dataset.cases.length, corpusEligible: eligible.length,
  corpusMissing: outcomes.filter((row) => row.corpusMissing).map((row) => row.id),
  before: beforeStats, after: afterStats, byFamily, outcomes, negativeControls: negatives,
  exactIdentityGuard: { checked: exactChecked, changes: exactChanges, reverseHandlerBypassed: true },
  regressions: outcomes.filter((row) => row.before?.rank != null &&
    (row.after?.rank == null || row.after.rank > row.before.rank)).map((row) => row.id),
  coverageCounts: count,
  boundary: 'Authored source-derived developer cases, not an independent clinical or open-ended semantic-search benchmark. Both readers use the same complete corpus; content growth is not credited as a ranking gain. Bounds are at the SQL adapter, not total process/device memory. Lexical absence cues do not establish patient state, diagnosis or calibrated confidence. No query-specific aliases or gold changes.',
};
database.close();
writeFileSync(outputPath, JSON.stringify(report, null, 2) + '\n');
console.log(JSON.stringify({ before: beforeStats, after: afterStats, byFamily,
  exactChecked, exactChanges, missing: report.corpusMissing, regressions: report.regressions }, null, 2));
if (exactChanges.length) throw new Error('Exact identity regression; report retained');
if (afterStats.top20 < beforeStats.top20) throw new Error('Aggregate reverse recall regressed; report retained');
if (negatives.some((row) => row.after.length > row.before.length)) throw new Error('Unrelated-query false positive increased; report retained');
