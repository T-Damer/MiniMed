...
if (!databasePath || !baselinePath || !outputPath)
  throw new Error('Usage: DB BASELINE_READER REPORT');
...
try {
  ...
  const eligible = values.filter(
    (name) => name.length <= 512 && !allNames.has(`что такое «${name}»?`),
  );
  ...
  const sample = eligible
    .map((name) => ({
      name,
      key: createHash('sha256').update(`question-v1:${name}`).digest('hex'),
    }))
  ...
  for (const item of sample) {
    ...
    outcomes.push({
      ...
      beforeMs: middle - started,
      afterMs: ended - middle,
    });
  }
  const report = {
    ...
    sample: {
      ...
      beforeExact: outcomes.filter((row) => row.beforeExact).length,
      afterExact: outcomes.filter((row) => row.afterExact).length,
      outcomes,
    },
    boundary:
      'Literal name-question framing evaluated against the complete prior exact-name output. ' +
  ...
  console.log(
    JSON.stringify({
      ...
      wrappedFtsCalls,
      sampleBefore: report.sample.beforeExact,
      sampleAfter: report.sample.afterExact,
      sampleCount: outcomes.length,
    }),
  );
  if (exactChanges.length || wrapperChanges.length || wrappedFtsCalls)
    throw new Error('Named-question boundary failed; report retained');
} finally {
  database.close();
}
