...
const normalize = (value) => value.normalize('NFKC').toLowerCase().replaceAll('ё', 'е').replace(/\s+/gu, ' ').trim();
const db = new Database(dbPath, { readonly: true });
const databaseSha256 = hash(readFileSync(dbPath));
const documents = db.query('SELECT id, title FROM documents').all();
const medicationDocuments = db.query("SELECT id, title FROM documents WHERE json_extract(metadata_json, '$.catalogFamily') = 'medication'").all();
const aliases = db.query('SELECT alias, canonical_term, category FROM aliases').all();
const known = new Set([...documents.map((row) => normalize(row.title)), ...aliases.flatMap((row) => [normalize(row.alias), normalize(row.canonical_term)])]);
...
const owners = [...targets.keys()].filter((name) => /^[а-я]{7,30}$/u.test(name)).map((name) => ({name, seed: hash(`medication-spelling-v1\0${name}`)})).sort((a, b) => a.seed.localeCompare(b.seed));
...
function corrupt(name, family, seed) {
  ...
  if (family === 'transposition') [letters[at], letters[at + 1]] = [letters[at + 1], letters[at]];
    ...
    for (let offset = 0; offset < letters.length && changed < required; offset += 1) {
      ...
      letters[position] = alternatives[Number.parseInt(seed.slice(8, 10), 16) % alternatives.length];
      changed += 1;
    }
...
for (const family of ['transposition', 'deletion', 'insertion', 'substitution', 'confusion-1', 'confusion-2', 'confusion-3']) {
  let count = 0;
  for (const owner of owners) {
    const query = corrupt(owner.name, family, owner.seed);
    if (!query || query === owner.name || seen.has(query)) continue;
    if (known.has(query)) { knownNameCollisionsExcluded += 1; continue; }
    seen.add(query);
    cases.push({ id: `${family}.${count}`, family, query, sourceName: owner.name, expected: targets.get(owner.name) });
    count += 1;
    if (count === 20) break;
  }
...
const authored = [
  ['парацетомол', 'парацетамол'], ['парацитамол', 'парацетамол'],
  ['парацитамолл', 'парацетамол'], ['парацетмаол', 'парацетамол'],
  ['парацтамол', 'парацетамол'], ['паарацетамол', 'парацетамол'],
  ['парацктамол', 'парацетамол'], ['пароцитамол', 'парацетамол'],
  ['цефтреаксон', 'цефтриаксон'], ['цефтриакзон', 'цефтриаксон'],
  ['цефазалин', 'цефазолин'], ['амоксицилин', 'амоксициллин'],
  ['омепрозол', 'омепразол'], ['азетромецин', 'азитромицин'],
  ['ациклавир', 'ацикловир'], ['резперидон', 'рисперидон'],
  ['энолоприл', 'эналаприл'], ['тиклофенак', 'диклофенак'],
...
].map(([query, name], i) => ({ id: `authored.${i}`, family: 'authored', query, sourceName: name, expected: targets.get(name) ?? [] }));
cases.push(...authored);
const exactQueries = [...new Set([...owners.slice(0, 100).map((row) => row.name), ...authored.map((row) => row.sourceName), 'кеторол', 'кетонал', 'клозапин', 'клоназепам'])];
const controls = ['АД', 'F20', '123456', 'космический мармелад', 'не парацитомол', 'аллергия на парацитомол'];
db.close();
...
async function sample(path) {
  ...
  try {
    for (const item of cases) {
      ...
      outcomes.push({ ...item, corpusMissing: item.expected.length === 0, rank: position < 0 ? null : position + 1, elapsedMs: result.elapsedMs, alternatives: result.analysis.branches.filter((branch) => branch.id.startsWith('medication-spelling-')).map((branch) => branch.label), top: result.groups.slice(0, 5).map((group) => ({id: group.documentId, title: group.title})) });
    }
    const exact = {};
    ...
    for (const query of controls) {
      ...
      negatives.push({query, spellingBranches: result.analysis.branches.filter((branch) => branch.id.startsWith('medication-spelling-')).length, ids: result.groups.map((group) => group.documentId)});
    }
    ...
    return { outcomes, exact, negatives, contextChecks, coldMs: cold.elapsedMs, documentFilterChecks: 2, analyses };
...
const summarize = (sample, family) => {
  ...
  return {total: rows.length, top1: rows.filter((row) => row.rank === 1).length, top5: rows.filter((row) => row.rank !== null && row.rank <= 5).length, top20: rows.filter((row) => row.rank !== null).length, p50Ms: times[Math.floor(times.length / 2)] ?? null, p95Ms: times[Math.min(times.length - 1, Math.floor(times.length * 0.95))] ?? null};
};
const exactChanges = exactQueries.filter((query) => JSON.stringify(before.exact[query]) !== JSON.stringify(after.exact[query]));
...
const report = {
  baselineCommit: '06b3c68de2725f124d15140daf317dc5400d199b', checkedCommit: commit,
  databaseSha256, caseSha256: hash(JSON.stringify(cases)), sameDatabaseForBoth: databaseSha256 === hash(readFileSync(dbPath)),
  corpus: {documents: documents.length, medicationDocuments: medicationDocuments.length, medicationAliases: aliases.filter((row) => row.category === 'medication').length, eligibleMechanicalNames: owners.length},
  cases: after.outcomes.map((row, i) => ({...row, beforeRank: before.outcomes[i].rank})),
  byFamily: Object.fromEntries([...new Set(cases.map((row) => row.family))].map((family) => [family, {before: summarize(before, family), after: summarize(after, family)}])),
  exactChecked: exactQueries.length, exactChanges, regressions, knownNameCollisionsExcluded,
...
assert(after.outcomes.filter((row) => row.rank !== null).length > before.outcomes.filter((row) => row.rank !== null).length);
