...
const health: StorageHealth = {
  schemaVersion: 2, sqliteVersion: 'test', fts5Available: true,
  contentPackIds: ['core'], documentCount: 1, backend: 'sqlite-wasm',
  persistent: true, installation: 'reused', sizeBytes: 4096,
};
const options = { url: 'https://example.invalid/core.db', databaseName: 'core.db', fetchTimeoutMs: 1000, poolName: 'consent-test' };
class FakeWorker {
  onmessage: ((event: MessageEvent<OpfsPackWorkerResponse>) => void) | undefined;
  onerror: (() => void) | undefined;
  terminate = vi.fn();
  postMessage = vi.fn((message: OpfsPackWorkerRequest) => {
    if (message.type === 'call' && message.method === 'close') this.emit({ id: message.id, result: undefined });
  });
  emit(data: OpfsPackWorkerResponse) { this.onmessage?.({ data } as MessageEvent<OpfsPackWorkerResponse>); }
}
function mockWorker(): FakeWorker[] {
  const instances: FakeWorker[] = [];
  vi.stubGlobal('Worker', class extends FakeWorker { constructor() { super(); instances.push(this); } });
  return instances;
}
...
describe('existing OPFS owner core download consent', () => {
  ...
  it('rejects declined consent and releases the owner without approving a transfer', async () => {
    const instances = mockWorker();
    const opened = WorkerOpfsMedicalStore.open(options, { requestDownload: async () => { throw new Error('Declined'); }, onProgress: vi.fn() });
