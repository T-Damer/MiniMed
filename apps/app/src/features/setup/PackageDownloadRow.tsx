...
import { contentModuleNeedsInstall, isModuleReleased } from '@/features/modules/local-packaged-modules';
...
export function PackageDownloadRow(props: {
  ...
  const task = createMemo(() => {
    props.revision;
    return props.runtime.listTasks().toReversed().find((item) => item.moduleId === props.module.id && item.version === props.module.version);
  });
  ...
  const active = () => {
    ...
    return starting() || (state !== undefined && !['completed', 'failed', 'cancelled'].includes(state));
  };
  const downloadable = () => { props.revision; return isModuleReleased(props.module); };
  const label = () => {
    ...
    if (!downloadable()) return props.module.releaseState === 'preview' ? 'Предварительный' : 'Недоступен';
    return props.module.sizes.downloadBytes === null ? 'По выбору' : formatModuleBytes(props.module.sizes.downloadBytes);
  };
  const install = async () => {
    ...
    try {
      ...
      else if (completed.state === 'failed') throw new Error(completed.errorMessage ?? 'Не удалось установить пакет.');
    } catch (cause) {
      if (!disposed) setError(cause instanceof Error ? cause.message : 'Не удалось установить пакет.');
    } finally {
      if (!disposed) setStarting(false);
    }
  };
    ...
    <li class="package-row" data-module-id={props.module.id}>
      <div class="package-row__copy">
        ...
        <Show when={props.module.definitionReference}>
          <p class="package-row__notice">Предварительный справочник: исходные записи требуют проверки.</p>
        </Show>
        <Show when={!downloadable() && !ready() && props.module.releaseState === 'preview'}>
          <p class="package-row__notice">Для подготовленного пакета нужны доступный файл и включённые экспериментальные модули.</p>
        </Show>
        <Show when={error()}>{(message) => <p class="package-row__error" role="alert">{message()}</p>}</Show>
      </div>
      <div class="package-row__status">
        <span class="package-row__state" role="status" aria-live="polite">{label()}</span>
        <Show when={active() && !ready()}>
          <progress class="package-row__progress" max={100}
            value={task()?.state === 'downloading' ? downloadPercent(task()?.downloadedBytes ?? 0, task()?.totalBytes) : undefined}
            aria-label={`Загрузка: ${props.module.title}`} />
          <Show when={task()}>{(current) => <button class="package-row__button" type="button" onClick={() => props.runtime.cancel(current().id)}>Отменить</button>}</Show>
        </Show>
