...
export function isSetupDismissed(): boolean {
  ...
  try { return localStorage.getItem(SETUP_DISMISSED_KEY) === '1'; }
  catch { return false; }
}

export function dismissSetup(): void {
  dismissedInSession = true;
  try { localStorage.setItem(SETUP_DISMISSED_KEY, '1'); }
  catch { /* Private browsing still retains the dismissal for this application session. */ }
}
...
export function setupPackageGroups(modules: readonly ContentModuleCatalogEntry[]) {
  ...
  return kinds.map(([kind, title, description]) => ({
    id: kind, title, description,
    modules: modules.filter((module) => module.kind === kind && !module.required && module.releaseState !== 'bundled')
      .toSorted((a, b) => Number(Boolean(b.definitionReference)) - Number(Boolean(a.definitionReference)) || a.title.localeCompare(b.title, 'ru')),
  })).filter((group) => group.modules.length > 0);
}

export function downloadPercent(loaded: number, total: number | null | undefined): number | undefined {
