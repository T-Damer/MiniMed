      ...
      <ConfirmationDialog
        ...
        description={
          deleteTarget()?.kind === 'card'
            ? `Карточка «${deleteTarget()?.title}», все её записи, файлы, изображения и расшифровки будут удалены с этого устройства. Перед удалением карточку можно экспортировать отдельной кнопкой. Отменить это действие нельзя.`
            : `Запись «${deleteTarget()?.title}» и её локальные файлы, изображения и расшифровки будут удалены с этого устройства. Отменить это действие нельзя.`
        }
