...
export function DefinitionReferenceAnnotations(props: {
  ...
  const [page, setPage] = createSignal<DefinitionReferenceAnnotationPage>({
    items: [],
    next: null,
  });
  ...
  const request = async (value: DefinitionReferenceRequest): Promise<DefinitionReferenceReply> => {
    ...
    if (!reply.ok || reply.value.op === 'unavailable')
      throw new Error('Редакция справочника больше не доступна.');
    return reply.value;
  };
  ...
  const load = async (after?: string) => {
    ...
    try {
      ...
      if (token === generation)
        setError('Не удалось прочитать пояснения. Определение остаётся доступным.');
    } finally {
  ...
  const showSource = async (item: DefinitionReferenceAnnotation) => {
    ...
    try {
      ...
      if (
        ...
        block.op !== 'text' ||
        !block.block ||
        block.block.sourceId !== item.sourceId
      )
        throw new Error('Source binding unavailable');
      if (token === generation)
        setProvenance({
          source: source.source,
          locator: block.block.provenance,
          span: { start: item.start, end: item.end, units: 'Unicode code points' },
        });
    } catch {
  ...
  return (
    <details
      class="reference-annotations"
      onToggle={(event) => setOpened(event.currentTarget.open)}
    >
      <summary class="reference-annotations__summary">
        Происхождение и исторические упоминания
      </summary>
      <Show when={opened()}>
        <p class="reference-annotations__notice">
          Дословные пояснения из связанных фрагментов источника. Требуют проверки; упоминание
          фамилии не устанавливает личность или приоритет открытия.
        </p>
        ...
        <For each={page().items}>
          {(item) => (
            <article class="reference-annotations__item">
              <h4 class="reference-annotations__title">
                {item.kind === 'etymology'
                  ? 'Происхождение: текст источника'
                  : `Имя в источнике: ${item.label}`}
              </h4>
        ...
        <Show when={provenance()}>
          <pre class="reference-card__metadata">{JSON.stringify(provenance(), null, 2)}</pre>
        </Show>
        <Show when={busy()}>
          <p class="reference-annotations__notice" role="status">
            Читаем пояснения…
          </p>
        </Show>
        <Show when={error()}>
