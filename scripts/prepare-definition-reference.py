...
def main() -> None:
    ...
    if database.exists() or archive.exists() or report_path.exists():
        raise ValueError('Use a new version or explicitly remove your obsolete DEV output first.')
    edition = 'minimed.definition.reference.' + args.version
    report = build_compact_definition_reference(
        inputs, database, input_root=root, edition_id=edition,
    ...
    module = {
        ...
        'description': record_label(expected_entries) + ': русские определения, описания, шкалы и контекст с точными ссылками. Предварительная редакция, требующая проверки.',
        ...
        'definitionReference': {'contract': 1, 'editionId': edition, 'entries': expected_entries},
    }
    ...
    print(json.dumps({'entries': expected_entries, 'installedBytes': database.stat().st_size, 'downloadBytes': archive.stat().st_size, 'sqliteSha256': checksum, 'publicationState': 'local-dev'}, indent=2))
