...
if (!path || !output || !expectedCount || !['corpus', 'fixture'].includes(mode)) {
  throw new Error(
    'Usage: bun scripts/verify-definition-reference.ts DATABASE REPORT EXPECTED_ENTRIES [corpus|fixture]',
  );
}
...
try {
  const reader = await createSqliteDefinitionReference({
    async read(statement, parameters) {
      calls++;
      const rows = database
        .query<Record<string, unknown>, (string | number)[]>(statement)
        .all(...parameters);
  ...
  const count = database
    .query<{ count: number }, []>('SELECT count(*) AS count FROM knowledge_entities')
    .get()?.count;
  assert.equal(count, Number(expectedCount));
  assert.equal(
    database.query<{ count: number }, []>('SELECT count(*) AS count FROM knowledge_facts').get()
      ?.count,
    0,
  );
  assert.equal(
    database.query<{ count: number }, []>('SELECT count(*) AS count FROM knowledge_relations').get()
      ?.count,
    0,
  );
  for (const limit of [0, -1, Number.NaN, Number.POSITIVE_INFINITY])
    assert.deepEqual(await reader.search('test', limit), []);
  ...
  if (mode === 'fixture') {
    ...
    fixtureAssertions++;
    assert.equal((await reader.search('ТТ')).length, 1);
    fixtureAssertions++;
    assert((await reader.search('точное определение')).some((item) => item.id === 'fixture.first'));
    fixtureAssertions++;
    assert.deepEqual(await reader.search('контекстмаркер'), []);
    fixtureAssertions++;
    assert.deepEqual(await reader.search('аннотациямаркер'), []);
    fixtureAssertions++;
    const page = await reader.listBlocks('fixture.first');
    assert.deepEqual(
      page.blocks.map((item) => item.role),
      ['definition', 'item', 'item', 'context', 'annotation'],
    );
    fixtureAssertions++;
    const block = page.blocks[0];
    assert(block);
    const content = await reader.readBlock('fixture.first', block.chunkId);
    assert(content);
    assert.equal(content.text, 'Точное определение тестового понятия.');
    fixtureAssertions++;
    assert.equal(content.provenance['locator'], 'Fixture paragraph 1');
    fixtureAssertions++;
    assert.equal(content.nextOffset, null);
    fixtureAssertions++;
    assert(await reader.getSource(content.sourceId));
    fixtureAssertions++;
    assert.equal(await reader.readBlock('fixture.missing', block.chunkId), null);
    fixtureAssertions++;
    await assert.rejects(reader.readBlock('fixture.first', block.chunkId, -1));
    fixtureAssertions++;
    await assert.rejects(reader.listBlocks('fixture.first', 'other.reference.000001'));
    fixtureAssertions++;
    const huge = await reader.getCard('fixture.long');
    assert(huge);
    let cursor: string | undefined;
    const all = [];
    do {
      const result = await reader.listBlocks(huge.id, cursor);
      assert(result.blocks.length <= 8);
      fixtureAssertions++;
      all.push(...result.blocks);
      cursor = result.next ?? undefined;
    } while (cursor);
    assert.equal(new Set(all.map((item) => item.linkId)).size, all.length);
    fixtureAssertions++;
    const longBlock = all.find((item) => item.role === 'definition');
    assert(longBlock);
    let offset = 0;
    let restored = '';
    for (;;) {
      const part = await reader.readBlock(huge.id, longBlock.chunkId, offset);
      assert(part);
    ...
    assert.equal(restored, '😀аб'.repeat(5000));
    fixtureAssertions++;
  }
  ...
  const report = {
    ...
    mode,
    entries: count,
    sourceNameSurfaces: names,
    namesWithAResult: names - missing.length,
    missingNameSha256: missing,
    fixtureAssertions,
    creationMs: openMs,
    lookupMs: {
    ...
    adapterCalls: calls,
    maxRowsPerAdapterCall: maxRows,
    maxReplyBytes,
    boundaries:
      'File-backed Bun SQLite in a fresh process, 2 MiB page-cache setting; not app/browser/native/WASM/device qualification. Name coverage is not independent reverse-definition quality. The corpus-name audit is outside the reader and runs in bounded batches. RSS is whole-process current memory, not a peak or Android PSS.',
  };
  assert(maxRows <= 20);
