...
from localmed_ingest.definition_source_manifest import describe_input, write_source_manifest
...
def checked_parts(directory: Path, manifest: dict[str, object]) -> list[Path]:
    ...
    for value in seq(manifest.get('parts'), 32):
        ...
        if (len(raw) != part.get('bytes')
                or hashlib.sha256(raw).hexdigest() != part.get('sha256')):
...
def refresh(root: Path, collection: Path) -> dict[str, object]:
    ...
    if collection.resolve().parent != source.resolve():
        raise ValueError('Collection must be a direct child of the public definition-source folder')
    old_paths = [source / name for name in (
        'catalog.json', 'ruwiktionary-2026.9.16.json', 'prepared-source-excerpts-2026.09.21.json',
    )]
    old_paths.extend(checked_parts(source, obj(json.loads((source / 'clinical-source-excerpts-2026.09.21.json').read_bytes()))))
    ...
    manifest = write_source_manifest(root, tuple(old_paths + new_paths), source / 'source-inputs.json')
    # Categories and same-title overlap describe source coverage, not clinical concept equivalence.
    report = {
        'schemaVersion': 1, 'previousBaseRecords': old_count, 'newSourceRecords': new_count,
        'combinedRecords': manifest['entries'], 'baseNormalizedNames': len(old_names),
        'newSourceNormalizedNames': len(new_names), 'newNameSurfaces': len(new_names - old_names),
        ...
        'previousProbeCoverage': {
            query: {'before': normalized_name(query) in old_names, 'after': normalized_name(query) in old_names | new_names}
            for query in ('Атаксия', 'Дисметрия', 'Ортопноэ', 'Брадипноэ')
        },
        'boundaries': 'New normalized name surfaces are not proven disjoint canonical concepts. Source text is not rewritten or reviewed. Old source-specific meanings remain separate. The owner textbook is not included in public counts. This operation replaces the previous encyclopedic source snapshot in the DEV manifest rather than accumulating duplicate page IDs.',
    }
...
def main() -> None:
    ...
    print(json.dumps({key: report[key] for key in ('previousBaseRecords', 'newSourceRecords', 'combinedRecords', 'newNameSurfaces', 'existingNameSurfacesWithAdditionalSource')}, indent=2))
