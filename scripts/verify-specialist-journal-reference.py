...
def verify(root: Path, selected: Path, database: Path, report_path: Path, cards_path: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix='journal-replay-') as directory:
        ...
        if replay != before:
            raise ValueError('Archived source replay changed the selected article projection')
        for value in seq(selection['articles'], 96):
            row = obj(value)
    ...
    paths, expected_count = read_source_manifest(root, root / 'content/definition-drafts/source-inputs.json')
    ...
    for value in seq(refresh['newInputs'], 32):
        ...
        new_ids.update(text(obj(term)['id'], 256) for term in seq(payload['terms'], 5000))
    ...
    with closing(sqlite3.connect(f'{database.resolve().as_uri()}?mode=ro', uri=True)) as db:
        ...
        actual_ids = {str(row[0]) for row in db.execute('SELECT id FROM knowledge_entities')}
        if actual_ids != set(projection.entries) or len(actual_ids) != expected_count:
            raise ValueError('SQLite identity set differs from selected source identities')
        if any(identifier.startswith('ruwiki.definition.') for identifier in actual_ids):
        ...
        for table in ('knowledge_facts', 'knowledge_relations'):
            if db.execute(f'SELECT count(*) FROM {table}').fetchone() != (0,):
                raise ValueError('Source extraction created clinical facts or relations')
        for chunk_id, (_, chunk) in projection.chunks.items():
            row = db.execute('SELECT original_text, metadata_json FROM chunks WHERE id=?', (chunk_id,)).fetchone()
            if row is None or row[0] != chunk.original_text or json.loads(row[1]) != chunk.metadata:
        ...
        for source in projection.sources.values():
            ...
            if row is None or obj(json.loads(row[0])).get('source') != source.descriptor:
                raise ValueError('Source descriptor, date or licensing changed in SQLite')
            checked_sources += 1
        for identifier in sorted(new_ids):
            ...
            if metadata.get('reviewStatus') != 'requires-review' or metadata.get('coverage') != entry.coverage:
            ...
            links = db.execute(
                ...
                'WHERE entity_id=? ORDER BY id', (identifier,),
            ).fetchall()
            if links != entry.links:
                raise ValueError('Source card lost or reordered its context blocks')
            checked_links += len(links)
            cards.append({
                'id': identifier, 'title': entry.title, 'coverage': entry.coverage,
                'blocks': [{
                    'chunkId': chunk_id, 'role': role,
                    'sourceId': projection.chunks[chunk_id][0].id,
                    'characters': len(projection.chunks[chunk_id][1].original_text),
                    'sha256': hashlib.sha256(projection.chunks[chunk_id][1].original_text.encode()).hexdigest(),
                } for role, chunk_id in entry.links],
            })
    report = {
        'selectedRecords': expected_count, 'newRecords': len(new_ids),
        'sourceBlocksCompared': len(projection.chunks), 'sourceDescriptorsCompared': checked_sources,
        'newCardLinksCompared': checked_links, 'selectedIdentitySetExact': True,
        'sourceTextAndMetadataExact': True, 'previousInputsUnchanged': True,
        'archivedArticleReplayExact': True, 'wikipediaRecords': 0,
        'approvedFactsOrRelations': 0, 'integrity': 'ok', 'foreignKeyViolations': 0,
        'networkRequests': 0,
