...
const fixture = spawnSync('uv', ['run', '--project', 'tools/ingest', 'python', '-c', `
import json
...
const descriptor = JSON.parse(await readFile(resolve(appRoot, 'src/features/modules/catalog.definition-reference.local.json'), 'utf8'));
assert.equal(descriptor.module.definitionReference.entries, 18133);
const archive = await readFile(resolve(appRoot, 'public/content/definition-reference', descriptor.fileName));
let coreGets = 0;
let referenceGets = 0;
let blockDatabaseReads = false;
const config = await loadConfigFromFile({ command: 'serve', mode: 'development' }, resolve(appRoot, 'vite.config.ts'));
assert(config);
...
const plugins = config.config.plugins.flat(Infinity).filter((plugin) => !plugin || !['ensure-tessdata-assets', 'ensure-pdfjs-assets', 'ensure-cornerstone-codec-assets'].includes(plugin.name));
plugins.push({
  ...
  configureServer(server) {
    server.middlewares.use((request, response, next) => {
      ...
      if (blockDatabaseReads) { response.statusCode = 503; response.end(); return; }
      ...
      const send = () => {
        ...
        if (offset === bytes.length) response.end(); else timer = setTimeout(send, delay);
      };
...
const server = await createServer({ ...config.config, configFile: false, root: appRoot, plugins, server: { ...config.config.server, host: '127.0.0.1', port: 5192, strictPort: true, proxy: {} } });
await server.listen();
...
async function measure(stage) {
  ...
  heap.push({ stage, usedSize: usage.usedSize, totalSize: usage.totalSize, embedderHeapUsedSize: usage.embedderHeapUsedSize, backingStorageSize: usage.backingStorageSize });
}
const report = { contract: 1, assertions: [], measurements: heap, boundaries: 'Headless Chromium, actual App with synthetic demo core and complete public local-dev reference. No native Android/device, independent clinical gold, OCR/viewer assets, whole-process peak or production release qualification. Cached restart blocks database downloads but still serves development JS and the core report.' };
...
try {
  ...
  await expect(coreRow.getByRole('button', { name: 'Скачать ядро', exact: true })).toBeVisible({ timeout: 60000 });
  ...
  const geometry = await setup.evaluate((element) => {
    ...
    return { width: rect.width, height: rect.height, viewportWidth: innerWidth, viewportHeight: innerHeight, scrollWidth: element.scrollWidth, bold: title ? Number(getComputedStyle(title).fontWeight) : 0 };
  });
  ...
  record('Fullscreen phone geometry, mandatory bold core and optional package list; no core GET before consent.');
  ...
  await page.waitForFunction(() => performance.getEntriesByName('minimed:search-ready').length > 0, { timeout: 90000 });
  assert.equal(coreGets, 1);
  assert.equal(await page.evaluate(() => localStorage.getItem('minimed:package-setup-dismissed:v1')), '1');
  ...
  await expect(referenceRow.getByText('Установлен', { exact: true })).toBeVisible({ timeout: 180000 });
  record('Full 18,133-entry optional edition installed through the normal download, gzip, checksum and index validator.');
  assert.equal(referenceGets, 1);
  // Core replacement may rebuild the search subtree; the same normal entry opens it again.
  if (await dictionary.count() === 0) await page.getByRole('button', { name: 'Словарь', exact: true }).click();
  ...
  await expect(dictionary.locator('.reference-panel__hit-button').first()).toBeVisible({ timeout: 30000 });
  ...
  record('Normal dictionary UI finds source records and reads bounded text, block descriptors and source citation.');
  await measure('reference-card-open');
  blockDatabaseReads = true;
  await page.reload({ waitUntil: 'domcontentloaded', timeout: 120000 });
  await page.waitForFunction(() => performance.getEntriesByName('minimed:search-ready').length > 0, { timeout: 120000 });
  ...
  await dictionary.getByRole('button', { name: 'Найти', exact: true }).click();
  await expect(dictionary.locator('.reference-panel__hit-button').first()).toBeVisible({ timeout: 30000 });
  record('Persisted dismissal and both installed databases survive application reload with database network reads blocked.');
  assert.equal(failures.length, 0, JSON.stringify(failures));
  assert.equal(external.length, 0, 'The tested path must not require external requests.');
  record('No uncaught page errors or external requests in the tested application path.');
  ...
  report.coreGets = coreGets; report.referenceGets = referenceGets;
  ...
  console.error('Visible app diagnostics:', (await page.locator('body').innerText()).slice(0, 16000));
  throw error;
} finally {
